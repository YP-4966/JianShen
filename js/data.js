// 健身房器械使用指南 - 动作数据（按训练部位分类）
// 动图来源：ExerciseDB 开源数据集（备份仓库 Johnson-Jia/exercises-dataset）

const CATEGORIES = [
  {
    id: "warmup",
    name: "热身训练",
    icon: "⚡",
    desc: "训练前激活全身",
    advice: "每次训练前先做 5-10 分钟热身：先低强度有氧提升体温，再做动态拉伸与关节活动，最后用轻重量热身组激活目标肌群，能显著降低受伤风险。"
  },
  {
    id: "chest",
    name: "胸部训练",
    icon: "💪",
    desc: "打造饱满胸肌",
    advice: "每周练 1-2 次；复合动作优先（卧推、推胸机），孤立动作（飞鸟）殿后；新手每组 8-12 次，共 3-4 组。"
  },
  {
    id: "shoulder",
    name: "肩部训练",
    icon: "🏋️",
    desc: "塑造立体肩部",
    advice: "肩关节灵活但易疲劳，每周 1-2 次；推举类在前，侧平举、反向飞鸟在后；建议小重量多次数（10-15 次）。"
  },
  {
    id: "back",
    name: "背部训练",
    icon: "🧗",
    desc: "练出倒三角",
    advice: "每周 1-2 次；先做垂直拉（引体、高位下拉），再做水平拉（划船类）；每组 8-12 次，注重顶峰收缩。"
  },
  {
    id: "glute",
    name: "臀部训练",
    icon: "🔥",
    desc: "翘臀塑形",
    advice: "可与腿部合练（臀腿日）；以髋伸动作（硬拉、臀桥）为主；全程感受臀部发力，避免腰部代偿。"
  },
  {
    id: "leg",
    name: "腿部训练",
    icon: "🦵",
    desc: "强壮腿部力量",
    advice: "腿部训练强度大，每周 1-2 次；深蹲、腿举等复合动作打头，孤立动作殿后；每组 8-12 次，循序渐进加重。"
  },
  {
    id: "arm",
    name: "手臂训练",
    icon: "🦾",
    desc: "手臂线条更清晰",
    advice: "小肌群恢复快，可每周 2 次；弯举（二头）与下压（三头）搭配练；每组 10-15 次。"
  },
  {
    id: "core",
    name: "核心训练",
    icon: "🧘",
    desc: "稳定核心力量",
    advice: "每次训练日可附加 10 分钟；卷腹与平板支撑交替；每组 15-20 次或保持 30-60 秒。"
  },
  {
    id: "cardio",
    name: "有氧训练",
    icon: "🏃",
    desc: "燃脂提升心肺",
    advice: "每次 20-40 分钟；强度以“能说话但微喘”为宜；训练前充分热身，结束后拉伸放松。"
  },
  {
    id: "stretch",
    name: "拉伸放松",
    icon: "🧘",
    desc: "训练后拉伸放松",
    advice: "训练结束后做 5-10 分钟静态拉伸，每个动作保持 20-30 秒，配合深长呼吸。所有拉伸均为站立或扶墙姿势、徒手单人即可完成，无需躺卧或他人协助。点上方「今天我练了哪个部位」选择本次训练部位，即可看到对应的推荐拉伸。"
  }
];

const EXERCISES = [
  // ==================== 热身训练 ====================
  {
    id: "jumping-jack",
    cat: "warmup",
    equipment: "徒手",
    level: "初级",
    name: "开合跳",
    en: "Jumping Jack",
    gif: "media/1g5bPpA.gif",
    target: ["心肺功能", "全身"],
    steps: [
      "双脚并拢站立，双臂自然下垂于身体两侧。",
      "跳起时双脚向两侧分开，同时双臂举过头顶。",
      "落地后迅速跳回起始姿势。",
      "保持节奏，持续跳跃至规定次数或时间。"
    ],
    tips: [
      "落地时膝盖微屈缓冲，前脚掌着地。",
      "保持均匀呼吸，节奏稳定。",
      "作为热身或间歇训练都很适合。"
    ],
    mistakes: [
      "落地过重、膝盖伸直锁死。",
      "手脚不同步，动作脱节。"
    ]
  },
  {
    id: "high-knees",
    cat: "warmup",
    equipment: "徒手",
    level: "初级",
    name: "高抬腿",
    en: "High Knees",
    gif: "media/ealLwvX.gif",
    target: ["心肺功能", "下肢"],
    steps: [
      "面向墙壁站立，双手扶墙保持平衡。",
      "核心收紧，快速交替将膝盖抬向胸部。",
      "保持上半身稳定，节奏由慢到快。",
      "持续至规定时间。"
    ],
    tips: [
      "抬膝时保持背部挺直，不要弯腰。",
      "从慢速开始，逐渐加快节奏。",
      "扶墙版适合新手，熟练后可原地进行。"
    ],
    mistakes: [
      "身体后仰、借力摆腿。",
      "膝盖抬得过高导致重心不稳。"
    ]
  },
  {
    id: "inchworm",
    cat: "warmup",
    equipment: "徒手",
    level: "初级",
    name: "毛毛虫爬行",
    en: "Inchworm",
    gif: "media/ZgsNQ6d.gif",
    target: ["腘绳肌", "核心", "肩部"],
    steps: [
      "站立，俯身弯腰，双手触地。",
      "双手向前爬行至高位平板支撑，身体呈一条直线。",
      "停顿片刻，双手移回双脚，尽量保持双腿伸直。",
      "站起还原，重复。"
    ],
    tips: [
      "全程核心收紧，背部保持平直。",
      "双腿尽量伸直，感受腘绳肌拉伸。",
      "是很好的全身动态拉伸动作。"
    ],
    mistakes: [
      "弓背含胸，腰部压力大。",
      "爬行时髋部左右晃动。"
    ]
  },
  {
    id: "mountain-climber",
    cat: "warmup",
    equipment: "徒手",
    level: "初级",
    name: "登山跑",
    en: "Mountain Climber",
    gif: "media/RJgzwny.gif",
    target: ["核心", "心肺功能"],
    steps: [
      "高位平板支撑，双手在肩部正下方，身体呈一条直线。",
      "核心收紧，交替将膝盖快速收向胸部。",
      "保持髋部稳定，节奏连贯。",
      "持续至规定时间。"
    ],
    tips: [
      "全程保持臀部不撅起、不塌腰。",
      "节奏越快强度越大，可快慢交替。",
      "配合呼吸，不要憋气。"
    ],
    mistakes: [
      "塌腰或撅臀。",
      "膝盖只小幅移动，动作幅度不足。"
    ]
  },
  {
    id: "glute-bridge-march",
    cat: "warmup",
    equipment: "徒手",
    level: "初级",
    name: "臀桥交替抬腿",
    en: "Glute Bridge March",
    gif: "media/GibBPPg.gif",
    target: ["臀大肌", "核心"],
    steps: [
      "仰卧，屈膝，双脚踩实地面。",
      "收紧臀部抬起髋部，身体呈一条直线。",
      "保持桥式，交替将单腿抬起、膝盖收向胸口。",
      "交替进行，保持髋部不塌。"
    ],
    tips: [
      "用臀部发力维持桥式，不要靠腰。",
      "抬腿时髋部保持稳定，不要左右歪斜。",
      "很好的臀部和核心激活动作。"
    ],
    mistakes: [
      "髋部下塌。",
      "腰部代偿、过度反弓。"
    ]
  },
  {
    id: "chest-shoulder-stretch",
    cat: "warmup",
    equipment: "徒手",
    level: "初级",
    name: "胸肩前侧拉伸",
    en: "Chest & Shoulder Stretch",
    gif: "media/Uto7l43.gif",
    target: ["胸肌", "肩部前侧"],
    steps: [
      "站立，双臂前伸与肩同高。",
      "双臂交叉，右臂在上左臂在下，双手交叉。",
      "手掌相贴，肩胛骨夹紧，双手向前推。",
      "保持 20-30 秒，放松后换手重复。"
    ],
    tips: [
      "拉伸感应在胸部和肩部前侧，不要耸肩。",
      "保持均匀呼吸，不要憋气。",
      "适合练胸、练肩前的热身与练后放松。"
    ],
    mistakes: [
      "耸肩、动作僵硬。",
      "拉伸过猛导致疼痛。"
    ]
  },
  {
    id: "ankle-circles",
    cat: "warmup",
    equipment: "徒手",
    level: "初级",
    name: "踝关节环绕",
    en: "Ankle Circles",
    gif: "media/uL9CsKm.gif",
    target: ["脚踝", "小腿"],
    steps: [
      "坐姿，双腿伸直于身前。",
      "抬起一条腿，以圆周方向转动脚踝。",
      "朝一个方向转若干圈，再换方向。",
      "换另一条腿重复。"
    ],
    tips: [
      "动作幅度以舒适为准，不要用力过猛。",
      "每天久坐后也可以做，帮助活动踝关节。",
      "深蹲、跑步前做，能有效预防踝部损伤。"
    ],
    mistakes: [
      "幅度太小，活动不充分。",
      "身体前倾借力。"
    ]
  },
  {
    id: "wrist-circles",
    cat: "warmup",
    equipment: "徒手",
    level: "初级",
    name: "腕关节环绕",
    en: "Wrist Circles",
    gif: "media/2zNKRUB.gif",
    target: ["手腕", "前臂"],
    steps: [
      "双臂向前伸直，双手握拳。",
      "保持手臂不动，以圆周方向转动手腕。",
      "正反方向交替进行。",
      "持续至手腕充分活动开。"
    ],
    tips: [
      "幅度轻柔，感受腕关节活动。",
      "适合在俯卧撑、卧推、提重物前做。",
      "办公久坐人群也可作为日常放松。"
    ],
    mistakes: [
      "手臂跟着晃动。",
      "转动过快、用力过猛。"
    ]
  },
  {
    id: "neck-side-stretch",
    cat: "warmup",
    equipment: "徒手",
    level: "初级",
    name: "颈侧拉伸",
    en: "Neck Side Stretch",
    gif: "media/x2chWLO.gif",
    target: ["颈部", "斜方肌"],
    steps: [
      "站直或坐直，肩膀放松下沉。",
      "将头缓慢向一侧倾斜，让耳朵靠近肩膀。",
      "保持 15-30 秒，感受颈侧拉伸。",
      "回到正中，换另一侧重复。"
    ],
    tips: [
      "不要耸肩，用另一侧手轻轻辅助即可。",
      "拉伸感在颈部侧面，出现疼痛立即停止。",
      "日常伏案工作后做，缓解颈部紧张。"
    ],
    mistakes: [
      "用力压头、幅度过大。",
      "身体歪斜代替头部倾斜。"
    ]
  },
  {
    id: "side-bend",
    cat: "warmup",
    equipment: "徒手",
    level: "初级",
    name: "体侧屈",
    en: "Side Bend",
    gif: "media/Hy9D21L.gif",
    target: ["腹斜肌", "侧腰"],
    steps: [
      "双脚与肩同宽站立，双臂自然下垂于体侧。",
      "保持背部挺直，身体向一侧缓慢弯曲，手沿大腿下滑。",
      "底部停顿片刻，缓慢回到起始位置。",
      "换另一侧交替进行。"
    ],
    tips: [
      "只做躯干侧屈，不要前后倾。",
      "感受侧腰拉伸，幅度以舒适为准。",
      "很日常的全身活动动作，适合训练前做。"
    ],
    mistakes: [
      "身体前倾或后仰。",
      "动作过快、借助惯性。"
    ]
  },
  {
    id: "walking-lunge",
    cat: "warmup",
    equipment: "徒手",
    level: "初级",
    name: "弓步走",
    en: "Walking Lunge",
    gif: "media/IZVHb27.gif",
    target: ["股四头肌", "臀大肌", "腘绳肌"],
    steps: [
      "双脚与肩同宽站立。",
      "向前迈一大步，下蹲至前腿大腿与地面平行。",
      "保持躯干直立，前膝与脚尖方向一致。",
      "前脚蹬地，后腿前迈继续弓步，交替前行。"
    ],
    tips: [
      "前膝不要内扣，与脚尖方向一致。",
      "上半身保持直立，不要前倾。",
      "作为热身可徒手进行，边走边充分伸展髋部。"
    ],
    mistakes: [
      "前膝过度前冲。",
      "后腿膝盖直接砸地。"
    ]
  },
  {
    id: "calf-stretch",
    cat: "warmup",
    equipment: "徒手",
    level: "初级",
    name: "小腿拉伸",
    en: "Calf Stretch",
    gif: "media/m0tCHqc.gif",
    target: ["小腿"],
    steps: [
      "面向墙壁站立，双手扶墙与肩同高。",
      "一只脚向后迈一步，脚后跟踩实地面，腿伸直。",
      "前腿屈膝，身体前倾，感受后腿小腿的拉伸。",
      "保持 20-30 秒，换腿重复。"
    ],
    tips: [
      "后腿脚后跟始终不离地。",
      "膝盖保持伸直，拉伸感更强。",
      "跑步、跳绳前做，预防小腿抽筋。"
    ],
    mistakes: [
      "脚后跟抬起，拉伸不到位。",
      "弓背、身体歪斜。"
    ]
  },
  {
    id: "short-stride-run",
    cat: "warmup",
    equipment: "徒手",
    level: "初级",
    name: "小步慢跑",
    en: "Short Stride Run",
    gif: "media/CcWEoWV.gif",
    target: ["全身", "心肺功能"],
    steps: [
      "身体站直，双手握拳自然摆臂，双脚自然站立。",
      "原地或向前做小步幅、快频率的慢跑。",
      "前脚掌先着地，膝盖微屈缓冲，保持腰背挺直。",
      "保持轻快节奏，持续 1-3 分钟。"
    ],
    tips: [
      "步幅要小、频率要快，肩膀放松不要耸肩。",
      "落地轻盈，膝盖始终微屈。",
      "适合作为训练前的第一项热身，让身体先热起来。"
    ],
    mistakes: [
      "步幅过大、身体后仰。",
      "脚后跟重重砸地。"
    ]
  },
  {
    id: "squat-overhead-reach",
    cat: "warmup",
    equipment: "徒手",
    level: "初级",
    name: "深蹲过顶伸展",
    en: "Squat to Overhead Reach",
    gif: "media/QChZi3x.gif",
    target: ["下肢", "肩背", "全身"],
    steps: [
      "双脚略宽于肩站立，脚尖微微外展。",
      "屈髋屈膝下蹲，至大腿与地面接近平行。",
      "起身时双臂顺势从体前向上伸展过头顶。",
      "再次下蹲，动作连贯重复。"
    ],
    tips: [
      "下蹲时膝盖与脚尖方向一致，腰背保持挺直。",
      "配合呼吸：下蹲吸气、起身呼气。",
      "以激活下肢与肩背为目的，幅度自然即可。"
    ],
    mistakes: [
      "膝盖内扣。",
      "起身时身体过度后仰。"
    ]
  },
  {
    id: "bear-crawl",
    cat: "warmup",
    equipment: "徒手",
    level: "初级",
    name: "熊爬",
    en: "Bear Crawl",
    gif: "media/0Yz8WdV.gif",
    target: ["核心", "肩背", "全身"],
    steps: [
      "双手双脚撑地，膝盖离地，臀部略高于肩。",
      "对侧手脚同时前移，保持四点支撑向前爬行。",
      "向前爬数步后，可后退回到起点。",
      "重复至完成计划距离或时间。"
    ],
    tips: [
      "全程收紧核心、背部平直，不要塌腰。",
      "脚步幅度不要过大，保持重心稳定。",
      "手腕不适者可改为膝盖着地降阶。"
    ],
    mistakes: [
      "塌腰或撅臀过高。",
      "同侧手脚一起移动（顺拐）。"
    ]
  },
  {
    id: "semi-squat-jump",
    cat: "warmup",
    equipment: "徒手",
    level: "初级",
    name: "半蹲跳",
    en: "Semi Squat Jump",
    gif: "media/6FMU51h.gif",
    target: ["下肢", "心肺功能"],
    steps: [
      "双脚与肩同宽站立，屈膝下蹲至半蹲位置。",
      "手臂向后摆动，用力向上跳起。",
      "落地时前脚掌先着地，顺势屈膝缓冲回到半蹲。",
      "连续进行跳跃。"
    ],
    tips: [
      "落地要轻，膝盖与脚尖方向一致。",
      "核心收紧，保持身体稳定不晃动。",
      "膝关节不适者降低跳跃高度或改为徒手深蹲。"
    ],
    mistakes: [
      "落地时膝盖内扣或锁死。",
      "跳跃时身体过度前倾。"
    ]
  },
  // ==================== 胸部训练 ====================
  {
    id: "shua-decline-chest-press",
    cat: "chest",
    equipment: "史密斯机",
    level: "初级",
    name: "舒华卧姿推胸训练器",
    en: "SHUA Decline Chest Press (SH-G7801)",
    gif: "media/ETZfAbZ.gif",
    target: ["胸大肌下束", "肱三头肌", "三角肌前束"],
    steps: [
      "将训练凳调至下斜角度，双脚卡住海绵滚轴固定。",
      "正握略宽于肩握杠，将杠铃从卡扣旋出。",
      "缓慢下放杠铃至下胸位置，肘部微收。",
      "呼气推起杠铃至手臂伸直（不锁死），重复至计划次数。"
    ],
    tips: [
      "史密斯机轨迹固定，适合安全冲重量。",
      "下放至下胸位置，肘部与身体约 45°。",
      "全程肩胛后收，不要耸肩。"
    ],
    mistakes: [
      "杠铃下放过低，肩部压力大。",
      "腰部离凳或臀部滑动。"
    ]
  },
  {
    id: "lever-seated-chest-press",
    cat: "chest",
    equipment: "固定器械",
    level: "初级",
    name: "坐式胸肌推举训练器",
    en: "Lever Seated Chest Press",
    gif: "media/T0yTjgW.gif",
    target: ["胸大肌", "肱三头肌", "三角肌前束"],
    steps: [
      "调整座椅高度，背部贴紧靠垫坐好，双脚踩实地面。",
      "正手握紧把手，肘部弯曲约 90°，挺胸、肩胛骨微收。",
      "呼气向前推起把手，直到手臂接近伸直。",
      "顶端稍停，吸气缓慢还原，感受胸肌被拉长。",
      "重复至计划次数。"
    ],
    tips: [
      "全程挺胸沉肩，用胸大肌发力而不是肩膀顶。",
      "推起至手臂接近伸直即可，不要锁死肘关节。",
      "座椅高度以把手与胸部中缝齐平为准。"
    ],
    mistakes: [
      "耸肩、塌胸，重量压在肩上。",
      "重量过大导致只推一半或身体离开靠垫。"
    ]
  },
  {
    id: "chest-press",
    cat: "chest",
    equipment: "固定器械",
    level: "初级",
    name: "坐姿推胸机",
    en: "Machine Chest Press",
    gif: "media/DOoWcnA.gif",
    target: ["胸大肌", "肱三头肌", "三角肌"],
    steps: [
      "调整座椅高度，使手柄与胸部中缝齐平，背部贴紧靠垫。",
      "正手握柄，肘部弯曲约 90°。",
      "呼气向前推起手柄，直到手臂接近伸直。",
      "顶端短暂停顿，吸气缓慢还原。",
      "重复至计划次数。"
    ],
    tips: [
      "挺胸、沉肩，把注意力放在胸肌发力上。",
      "推起时手臂伸直即可，不要锁死肘关节。",
      "初学者先空杆或小重量熟悉轨迹。"
    ],
    mistakes: [
      "耸肩、胸部塌陷，重量压在肩膀上。",
      "重量过大导致身体晃动、只推一半。"
    ]
  },
  {
    id: "pec-deck",
    cat: "chest",
    equipment: "固定器械",
    level: "初级",
    name: "蝴蝶机夹胸",
    en: "Pec Deck",
    gif: "media/v3xmPAR.gif",
    target: ["胸大肌", "三角肌前束"],
    steps: [
      "调整座椅高度，背部贴紧靠垫。",
      "双手握住把手，肘部保持微屈固定。",
      "呼气将把手向胸前合拢，挤压胸肌。",
      "顶峰收缩 1-2 秒，吸气缓慢还原，感受胸肌拉伸。",
      "重复至计划次数。"
    ],
    tips: [
      "全程肘部角度不变，用胸部收缩带动合拢。",
      "顶峰停顿挤压感要明显，宁轻勿重。",
      "还原速度放慢，避免惯性弹回。"
    ],
    mistakes: [
      "用手臂力量“夹”，而不是用胸肌发力。",
      "还原过快、借助惯性弹回，胸肌刺激不足。"
    ]
  },
  {
    id: "smith-machine-bench-press",
    cat: "chest",
    equipment: "史密斯机",
    level: "初级",
    name: "史密斯卧推",
    en: "Smith Machine Bench Press",
    gif: "media/trqKQv2.gif",
    target: ["胸大肌", "肱三头肌"],
    steps: [
      "将平板凳置于史密斯杠下方，杠降至胸口上方约一拳处。",
      "仰卧，双脚踩实地面，双手略宽于肩握杠。",
      "解锁杠铃，控制下放至胸部中下沿。",
      "呼气向上推起至手臂接近伸直，重复。"
    ],
    tips: [
      "提前调好安全挡杆高度，力竭时能接住杠铃更安全。",
      "下放时手肘与身体约呈 45°-75°，不要过度外展。",
      "肩胛后收下沉，挺胸不塌腰。"
    ],
    mistakes: [
      "用杠铃弹胸借力。",
      "手肘过度外展，给肩关节带来压力。"
    ]
  },
  {
    id: "lever-incline-chest-press",
    cat: "chest",
    equipment: "固定器械",
    level: "初级",
    name: "坐姿上斜推胸机",
    en: "Lever Incline Chest Press",
    gif: "media/o17Jfkt.gif",
    target: ["胸大肌上束", "肱三头肌"],
    steps: [
      "调整座椅高度，使把手与胸部中上位置齐平。",
      "背部贴紧靠垫坐稳，双手握把。",
      "呼气将把手向前上方推出，至手臂接近伸直。",
      "吸气缓慢还原，感受上胸拉伸，重复。"
    ],
    tips: [
      "全程挺胸沉肩，不要耸肩借力。",
      "推起时想象用上胸发力，顶端不锁死手肘。",
      "还原时保持控制，重量以能完成 8-12 次为宜。"
    ],
    mistakes: [
      "肩膀耸起、含胸弓背。",
      "还原过快，配重片相互碰撞。"
    ]
  },
  {
    id: "barbell-bench-press",
    cat: "chest",
    equipment: "杠铃",
    level: "中级",
    name: "杠铃卧推",
    en: "Barbell Bench Press",
    gif: "media/EIeI8Vf.gif",
    target: ["胸大肌", "肱三头肌", "三角肌前束"],
    steps: [
      "平躺于卧推凳，双脚踩实地面，肩胛骨后缩下沉。",
      "正手握杠，握距略宽于肩，将杠铃从架上举起至胸部上方。",
      "缓慢下放杠铃至胸部中下沿，肘部约 45° 内收。",
      "底部停顿片刻，用力推起回到起始位置。",
      "重复至计划次数，大重量务必请人保护。"
    ],
    tips: [
      "挺胸收腹，肩胛骨全程贴紧凳面。",
      "手腕保持中立，杠铃压在掌根。",
      "大重量训练建议使用护杠或找同伴保护。"
    ],
    mistakes: [
      "手腕过度后折，前臂代偿受力。",
      "臀部离开凳面、腰部过度弓起。"
    ]
  },
  {
    id: "dumbbell-bench-press",
    cat: "chest",
    equipment: "哑铃",
    level: "初级",
    name: "哑铃卧推",
    en: "Dumbbell Bench Press",
    gif: "media/SpYC0Kp.gif",
    target: ["胸大肌", "肱三头肌", "三角肌前束"],
    steps: [
      "平躺于卧推凳，双手持哑铃于大腿上方，用膝盖辅助踢起至胸部上方。",
      "手掌朝前，双臂伸直至胸部上方。",
      "缓慢下放哑铃至胸部两侧，肘部呈约 90°。",
      "底部停顿片刻，用力推起还原。",
      "重复至计划次数，结束时小心收哑铃。"
    ],
    tips: [
      "哑铃轨迹比杠铃更灵活，活动幅度更大。",
      "保持手腕中立，小臂垂直地面。",
      "下放深度以肩部舒适为准，不要过度拉伸。"
    ],
    mistakes: [
      "哑铃晃动、轨迹不稳定。",
      "下放过低过度拉伸肩关节前侧。"
    ]
  },
  {
    id: "dumbbell-fly",
    cat: "chest",
    equipment: "哑铃",
    level: "中级",
    name: "哑铃飞鸟",
    en: "Dumbbell Fly",
    gif: "media/yz9nUhF.gif",
    target: ["胸大肌", "三角肌前束"],
    steps: [
      "平躺在长凳上，双手各握一个哑铃，掌心相对。",
      "将手臂伸直至胸部上方，肘部微屈固定。",
      "保持肘部微屈，以宽弧线将手臂向两侧放低，直到胸部有拉伸感。",
      "底部停顿片刻，沿原轨迹将哑铃拉回起始位置。",
      "重复至计划次数。"
    ],
    tips: [
      "全程保持肘部固定微屈角度，用胸肌控制动作。",
      "下放幅度以胸部拉伸感为准，不要过度下沉。",
      "重量宁轻勿重，飞鸟是孤立动作，重在感受收缩。"
    ],
    mistakes: [
      "把飞鸟做成卧推，肘部弯曲过大、变成推举。",
      "下放过低，肩关节过度拉伸。"
    ]
  },
  {
    id: "push-up",
    cat: "chest",
    equipment: "徒手",
    level: "初级",
    name: "俯卧撑",
    en: "Push-Up",
    gif: "media/I4hDWkc.gif",
    target: ["胸大肌", "肱三头肌", "核心"],
    steps: [
      "高位平板支撑起始，双手略宽于肩，身体呈一条直线。",
      "屈肘下沉身体，肘部与躯干约呈 45°。",
      "胸部下放至贴近地面，核心全程收紧。",
      "底部停顿片刻，撑起还原。",
      "重复至计划次数。"
    ],
    tips: [
      "身体从头顶到脚跟保持一条直线，不要塌腰撅臀。",
      "下落时吸气，撑起时呼气。",
      "力量不足可从跪姿俯卧撑或上斜俯卧撑开始。"
    ],
    mistakes: [
      "塌腰或撅臀，腰椎受力。",
      "耸肩、肘部过度外展，肩部压力大。"
    ]
  },
  {
    id: "chest-dip",
    cat: "chest",
    equipment: "单双杠",
    level: "高级",
    name: "双杠臂屈伸",
    en: "Chest Dip",
    gif: "media/XgWyAiA.gif",
    target: ["胸大肌", "肱三头肌", "三角肌前束"],
    steps: [
      "双手撑在双杠上，手臂伸直，身体略前倾。",
      "屈膝收腿，保持躯干前倾、肩部下沉。",
      "屈肘缓慢下放身体，直到肩部略低于肘部、胸部有拉伸感。",
      "发力撑起还原，避免耸肩。",
      "重复至计划次数。"
    ],
    tips: [
      "身体前倾越多，胸肌参与越多；身体越直立，越偏重三头肌。",
      "下放幅度以肩部舒适为准，不要过度。",
      "力量不足可使用辅助器械或弹力带降阶。"
    ],
    mistakes: [
      "肩部耸起、肘部过度外展，肩关节压力大。",
      "只做半程，动作幅度不够。"
    ]
  },
  {
    id: "cable-fly",
    cat: "chest",
    equipment: "绳索",
    level: "中级",
    name: "绳索夹胸",
    en: "Cable Fly",
    gif: "media/Pr9Rhf4.gif",
    target: ["胸大肌", "三角肌前束"],
    steps: [
      "将滑轮调整至胸部高度，双手握手柄，背对器械站立。",
      "向前迈一步站稳，肘部微屈，身体略前倾。",
      "用胸肌发力将双臂向前夹拢至胸前。",
      "顶峰挤压胸肌 1 秒，缓慢还原，感受胸部拉伸。",
      "重复至计划次数。"
    ],
    tips: [
      "肘部角度全程固定，用胸肌发力而不是手臂。",
      "身体保持稳定，不要借身体摆动。",
      "滑轮调低练上胸，调高练下胸，可灵活切换。"
    ],
    mistakes: [
      "手臂屈伸，把夹胸做成推举。",
      "还原过快，靠惯性弹回。"
    ]
  },
  {
    id: "assisted-chest-dip",
    cat: "chest",
    equipment: "固定器械",
    level: "初级",
    name: "辅助双杠臂屈伸",
    en: "Assisted Chest Dip",
    gif: "media/PAgTVaK.gif",
    target: ["胸大肌", "肱三头肌", "三角肌"],
    steps: [
      "调整辅助重量，双膝跪在垫板上，双手正握把手。",
      "身体前倾约 15-30°，肘部向两侧打开，缓慢屈肘下降。",
      "下降至上臂与地面平行，感受胸部拉伸。",
      "呼气推起还原至手臂伸直，重复至计划次数。"
    ],
    tips: [
      "身体前倾幅度越大，胸部参与越多。",
      "辅助重量从大到小渐进，保持动作标准。",
      "下降速度放慢，底部不要弹起。"
    ],
    mistakes: [
      "身体过直，变成练三头的臂屈伸。",
      "耸肩或肘部过度外展，肩部压力大。"
    ]
  },
  {
    id: "smith-incline-bench-press",
    cat: "chest",
    equipment: "史密斯机",
    level: "中级",
    name: "史密斯上斜卧推",
    en: "Smith Incline Bench Press",
    gif: "media/5v7KYld.gif",
    target: ["胸大肌上束", "三角肌前束", "肱三头肌"],
    steps: [
      "将训练凳调至 30-45° 上斜，背部贴紧凳面，双脚踩实。",
      "正握略宽于肩握杠，将杠铃从卡扣旋出。",
      "缓慢下放杠铃至锁骨附近，肘部微收。",
      "呼气推起杠铃至手臂伸直（不锁死），重复至计划次数。"
    ],
    tips: [
      "轨道固定，可专注上胸发力。",
      "下放至锁骨或上胸位置，不要过低。",
      "全程肩胛后收下沉，避免耸肩。"
    ],
    mistakes: [
      "杠铃下放过低，肩部前侧压力大。",
      "腰部拱起离凳，用腰部推重量。"
    ]
  },
  {
    id: "machine-inner-chest-press",
    cat: "chest",
    equipment: "固定器械",
    level: "初级",
    name: "内侧胸推机",
    en: "Machine Inner Chest Press",
    gif: "media/wDN97Ca.gif",
    target: ["胸大肌", "肱三头肌", "三角肌前束"],
    steps: [
      "调整座椅高度，背部贴紧靠垫，双手正握把手。",
      "手肘保持约 90°，肩胛骨微收。",
      "呼气向前推起把手，双手向身体中线靠拢。",
      "顶端略停挤压胸缝，吸气缓慢还原，重复。"
    ],
    tips: [
      "推起时双手向中间收拢，强化胸中缝刺激。",
      "全程挺胸沉肩，不要耸肩。",
      "还原幅度以胸部有拉伸感为准。"
    ],
    mistakes: [
      "耸肩、塌胸，重量压在肩上。",
      "推起过快，顶峰无挤压停顿。"
    ]
  },
  // ==================== 肩部训练 ====================
  {
    id: "shua-seated-shoulder-press",
    cat: "shoulder",
    equipment: "史密斯机",
    level: "初级",
    name: "舒华卧式推肩训练器",
    en: "SHUA Seated Shoulder Press (SH-G7805)",
    gif: "media/xUwnBMT.gif",
    target: ["三角肌", "肱三头肌", "上背部"],
    steps: [
      "调整座椅高度使握把与肩齐平，背部贴紧靠垫。",
      "正握把手推离卡扣，手臂伸直但不锁死。",
      "缓慢下放至肩部高度，肘部微向前。",
      "呼气推起至头顶，顶端略停，缓慢下放重复。"
    ],
    tips: [
      "史密斯机轨迹垂直固定，可安全上重量。",
      "全程挺胸沉肩，背部贴紧靠垫。",
      "下放至肩部高度即可，不要过低。"
    ],
    mistakes: [
      "耸肩或用斜方肌代偿推起。",
      "腰部离凳拱起借力。"
    ]
  },
  {
    id: "machine-shoulder-press",
    cat: "shoulder",
    equipment: "固定器械",
    level: "初级",
    name: "坐姿器械推举",
    en: "Machine Shoulder Press",
    gif: "media/67n3r98.gif",
    target: ["三角肌", "肱三头肌"],
    steps: [
      "调整座椅高度，背部贴紧靠背。",
      "正手握柄，双手置于肩部高度。",
      "向上推起手柄至手臂伸直，但不锁死肘关节。",
      "顶端停顿片刻，缓慢下放还原。",
      "重复至计划次数。"
    ],
    tips: [
      "背部贴紧靠垫，避免腰部悬空。",
      "推起轨迹保持竖直，不要偏向两侧。",
      "器械轨迹固定，适合新手安全练肩。"
    ],
    mistakes: [
      "耸肩推起，斜方肌代偿。",
      "推起时身体离开靠垫。"
    ]
  },
  {
    id: "lever-seated-reverse-fly",
    cat: "shoulder",
    equipment: "固定器械",
    level: "初级",
    name: "坐姿反向飞鸟机",
    en: "Lever Seated Reverse Fly",
    gif: "media/myfUsKf.gif",
    target: ["三角肌后束", "上背"],
    steps: [
      "面对器械坐好，胸部贴住靠垫，双手握两侧把手。",
      "呼气，双臂向身体两侧打开，感受肩后束收缩。",
      "顶端略停，吸气缓慢还原，重复。"
    ],
    tips: [
      "手肘微屈并固定角度，用肩发力而非手臂。",
      "向后打开时不要耸肩。",
      "重量宜轻，重点感受后束收缩。"
    ],
    mistakes: [
      "动作幅度过大，借助身体晃动。",
      "耸肩或斜方肌代偿过多。"
    ]
  },
  {
    id: "barbell-military-press",
    cat: "shoulder",
    equipment: "杠铃",
    level: "中级",
    name: "杠铃站姿推举",
    en: "Barbell Military Press",
    gif: "media/wdRZISl.gif",
    target: ["三角肌", "肱三头肌"],
    steps: [
      "双脚与肩同宽站立，正手握杠，握距略窄于肩。",
      "将杠铃举至肩部高度，肘部微微前置。",
      "收紧核心，将杠铃垂直推过头顶，双臂伸直。",
      "顶端停顿片刻，缓慢下放回肩部高度。",
      "重复至计划次数。"
    ],
    tips: [
      "核心和臀部收紧，避免腰部过度后仰。",
      "杠铃轨迹贴近面部垂直向上。",
      "腰部不适者改用坐姿推举。"
    ],
    mistakes: [
      "过度挺腰后仰，用腰部“推”重量。",
      "下放时肘部过度外展，肩部压力大。"
    ]
  },
  {
    id: "dumbbell-shoulder-press",
    cat: "shoulder",
    equipment: "哑铃",
    level: "初级",
    name: "哑铃肩上推举",
    en: "Dumbbell Overhead Press",
    gif: "media/A6wtbuL.gif",
    target: ["三角肌", "肱三头肌"],
    steps: [
      "双脚与肩同宽站立（或坐姿），双手持哑铃举至肩高，手掌朝前。",
      "核心收紧，骨盆保持中立。",
      "呼气将哑铃向上推起，直至手臂在头顶伸直。",
      "顶端停顿片刻，缓慢下放回肩部高度。",
      "重复至计划次数。"
    ],
    tips: [
      "收紧核心，避免腰椎过度前凸后仰。",
      "坐姿推举可减少腰部负担，更孤立肩部。",
      "推起时哑铃略微向头顶上方靠拢，轨迹更稳。"
    ],
    mistakes: [
      "过度挺腰后仰，用腰部“推”重量。",
      "耸肩推起，肩颈压力大。"
    ]
  },
  {
    id: "dumbbell-lateral-raise",
    cat: "shoulder",
    equipment: "哑铃",
    level: "初级",
    name: "哑铃侧平举",
    en: "Dumbbell Lateral Raise",
    gif: "media/DsgkuIt.gif",
    target: ["三角肌中束"],
    steps: [
      "双脚与肩同宽站立，双手持哑铃于体侧，掌心相对。",
      "背部挺直，核心收紧。",
      "肘部微屈，将双臂向两侧抬起至与地面平行。",
      "顶端停顿片刻，缓慢下放还原。",
      "重复至计划次数。"
    ],
    tips: [
      "肩部发力，不要耸肩或借力甩动。",
      "手腕保持中立，顶端可微旋手腕强化中束刺激。",
      "小重量多组数（12-15 次）效果更好。"
    ],
    mistakes: [
      "耸肩，斜方肌过度代偿。",
      "甩动身体借力，把侧平举做成“摆荡”。"
    ]
  },
  {
    id: "dumbbell-reverse-fly",
    cat: "shoulder",
    equipment: "哑铃",
    level: "初级",
    name: "哑铃反向飞鸟",
    en: "Dumbbell Reverse Fly",
    gif: "media/EAs3xL9.gif",
    target: ["三角肌后束", "上背部"],
    steps: [
      "双脚与肩同宽站立，双手持哑铃于体前，掌心相对。",
      "微屈膝，髋部前倾俯身，背部挺直。",
      "肘部微屈，将双臂向两侧展开至与地面平行。",
      "顶端停顿挤压后束与上背，缓慢还原。",
      "重复至计划次数。"
    ],
    tips: [
      "俯身角度约 45°，保持背部平直。",
      "用后束与上背发力，不要靠腰部晃动。",
      "重量要轻，重点是感受后束收缩。"
    ],
    mistakes: [
      "弓背含胸，腰部压力大。",
      "靠惯性甩臂，刺激不足。"
    ]
  },
  {
    id: "dumbbell-front-raise",
    cat: "shoulder",
    equipment: "哑铃",
    level: "初级",
    name: "哑铃前平举",
    en: "Dumbbell Front Raise",
    gif: "media/3eGE2JC.gif",
    target: ["三角肌前束"],
    steps: [
      "双脚与肩同宽站立，双手持哑铃于体前，掌心朝后。",
      "保持手臂微屈，将哑铃举至与肩齐平。",
      "顶端停顿片刻，缓慢下放还原。",
      "重复至计划次数。"
    ],
    tips: [
      "不要用惯性甩起，控制动作节奏。",
      "前束在推举中参与较多，此动作重量要轻。",
      "可左右交替进行，减少腰部代偿。"
    ],
    mistakes: [
      "身体后仰借力。",
      "耸肩、举得过高超过肩部水平。"
    ]
  },
  {
    id: "barbell-upright-row",
    cat: "shoulder",
    equipment: "杠铃",
    level: "初级",
    name: "杠铃直立划船",
    en: "Barbell Upright Row",
    gif: "media/UDlhcO8.gif",
    target: ["三角肌", "斜方肌", "肱二头肌"],
    steps: [
      "双脚与肩同宽站立，正手握杠，握距略宽于肩。",
      "杠铃自然垂于大腿前，双臂伸直。",
      "呼气，以肘部带动，将杠铃沿身体向上提拉至下巴高度。",
      "顶端略停，吸气缓慢下放还原。",
      "重复至计划次数。"
    ],
    tips: [
      "手肘始终高于手腕，用肘部引领上提。",
      "杠铃贴近身体上滑，不要远离躯干。",
      "握距太窄易挤压肩关节，略宽于肩更安全。"
    ],
    mistakes: [
      "耸肩、斜方肌过度发力。",
      "身体前后晃动借力。",
      "上提过高超过下巴，肩部压力大。"
    ]
  },
  {
    id: "cable-lateral-raise",
    cat: "shoulder",
    equipment: "绳索",
    level: "初级",
    name: "绳索侧平举",
    en: "Cable Lateral Raise",
    gif: "media/goJ6ezq.gif",
    target: ["三角肌中束"],
    steps: [
      "站立于绳索龙门架中央，双手各握一侧把手（或单侧站姿交替）。",
      "双臂微屈垂于体侧，核心收紧。",
      "呼气，双臂向两侧抬起至与地面平行。",
      "顶端停顿片刻，吸气缓慢下放还原。",
      "重复至计划次数。"
    ],
    tips: [
      "绳索全程保持张力，比哑铃更能持续刺激中束。",
      "肘部微屈固定角度，不要耸肩。",
      "重量宜轻，控制节奏避免甩动。"
    ],
    mistakes: [
      "耸肩或斜方肌代偿。",
      "靠惯性甩臂，顶峰无停顿。"
    ]
  },
  {
    id: "cable-rear-delt-row",
    cat: "shoulder",
    equipment: "绳索",
    level: "初级",
    name: "绳索面拉",
    en: "Cable Rear Delt Row",
    gif: "media/wqNPGCg.gif",
    target: ["三角肌后束", "斜方肌", "菱形肌"],
    steps: [
      "将绳索把手挂在低位滑轮上，面对器械站立。",
      "双手正握绳索两端，微屈膝、髋部前倾，背部挺直。",
      "呼气，将绳索拉向面部方向，肘部向两侧打开，挤压肩胛。",
      "顶端略停，吸气缓慢还原，重复。"
    ],
    tips: [
      "用后束与上背发力，而不是手臂。",
      "肘部抬高与肩同高，双手末端向耳后拉开。",
      "重量要轻，重点感受后束与肩胛收缩。"
    ],
    mistakes: [
      "耸肩或身体后仰借力。",
      "拉向胸部而非面部，练成划船。"
    ]
  },
  {
    id: "dumbbell-arnold-press",
    cat: "shoulder",
    equipment: "哑铃",
    level: "中级",
    name: "哑铃阿诺德推举",
    en: "Dumbbell Arnold Press",
    gif: "media/Xy4jlWA.gif",
    target: ["三角肌", "肱三头肌", "上胸"],
    steps: [
      "坐于有靠背的凳上，双手持哑铃于肩前，掌心朝向自己。",
      "呼气，向上推举的同时旋转手腕，使掌心在顶端朝前。",
      "手臂接近伸直时略停，感受肩部收缩。",
      "吸气，按原轨迹缓慢下放并回旋手腕还原。",
      "重复至计划次数。"
    ],
    tips: [
      "推举与旋转要连贯流畅，不要脱节。",
      "全程背部贴紧靠垫，避免腰部后仰。",
      "动作旋转幅度以舒适为准，肩部不适可减小旋转。"
    ],
    mistakes: [
      "挺腰后仰借力推起。",
      "旋转过猛导致肩关节不适。"
    ]
  },
  {
    id: "lever-lateral-raise",
    cat: "shoulder",
    equipment: "固定器械",
    level: "初级",
    name: "坐姿器械侧平举",
    en: "Lever Lateral Raise",
    gif: "media/dRTfGZT.gif",
    target: ["三角肌中束"],
    steps: [
      "调整座椅高度，背部贴紧靠垫坐好。",
      "双手握把手，手臂伸直或微屈，掌心朝下。",
      "呼气，双臂向两侧抬起至与地面平行。",
      "顶端停顿片刻，吸气缓慢下放还原。",
      "重复至计划次数。"
    ],
    tips: [
      "器械轨迹固定，适合孤立训练中束。",
      "全程控制速度，下放时不要松劲。",
      "肩部放松下沉，不要耸肩。"
    ],
    mistakes: [
      "耸肩或身体离开靠垫借力。",
      "上抬过快，靠惯性完成。"
    ]
  },
  {
    id: "smith-shoulder-press",
    cat: "shoulder",
    equipment: "史密斯机",
    level: "初级",
    name: "史密斯肩推",
    en: "Smith Shoulder Press",
    gif: "media/903mzG8.gif",
    target: ["三角肌", "肱三头肌"],
    steps: [
      "调整座椅高度，杠铃杆置于肩部高度，双脚踩实地面。",
      "正握略宽于肩握杠，将杠铃从卡扣中旋出。",
      "呼气，沿轨道垂直推起杠铃至头顶，手臂接近伸直。",
      "顶端略停，吸气缓慢下放至肩部高度。",
      "重复至计划次数。"
    ],
    tips: [
      "轨道固定，推举轨迹更稳，适合上大重量。",
      "全程背部贴紧靠垫，避免腰部过度后仰。",
      "下放至下巴或耳垂高度即可，不要过低。"
    ],
    mistakes: [
      "挺腰后仰，用腰部推重量。",
      "下放过低导致肩部前侧压力大。"
    ]
  },
  {
    id: "lever-military-press",
    cat: "shoulder",
    equipment: "固定器械",
    level: "初级",
    name: "坐姿器械军事推举",
    en: "Lever Military Press",
    gif: "media/CggQhII.gif",
    target: ["三角肌", "肱三头肌", "上胸"],
    steps: [
      "调整座椅高度，背部贴紧靠垫，双脚踩实地面。",
      "正握把手，握距略宽于肩，置于肩部高度。",
      "呼气向上推起把手至手臂接近伸直，不锁死肘关节。",
      "顶端略停，吸气缓慢下放还原，重复。"
    ],
    tips: [
      "器械轨迹固定，适合安全冲重量。",
      "全程挺胸沉肩，不要耸肩推起。",
      "推起至手臂接近伸直即可，不要过度锁肘。"
    ],
    mistakes: [
      "耸肩或斜方肌代偿推起。",
      "下放过快，靠惯性完成。"
    ]
  },
  {
    id: "smith-rear-delt-row",
    cat: "shoulder",
    equipment: "史密斯机",
    level: "初级",
    name: "史密斯后束划船",
    en: "Smith Rear Delt Row",
    gif: "media/nFUwqG6.gif",
    target: ["三角肌后束", "斜方肌", "菱形肌"],
    steps: [
      "调整座椅高度，胸口贴住靠垫坐好，双脚踩实。",
      "正握把手，握距略宽于肩，手臂自然下垂。",
      "呼气，将把手拉向胸部两侧，肘部外展，挤压肩胛。",
      "顶端略停，吸气缓慢还原，重复。"
    ],
    tips: [
      "用后束与上背发力，不要用手臂硬拉。",
      "肘部向两侧打开，而不是向后贴体。",
      "重量宜轻，重点是感受后束收缩。"
    ],
    mistakes: [
      "耸肩或斜方肌过度代偿。",
      "身体后仰或离开靠垫借力。"
    ]
  },
  // ==================== 背部训练 ====================
  {
    id: "shua-narrow-seated-row",
    cat: "back",
    equipment: "固定器械",
    level: "初级",
    name: "舒华斜式坐姿划船训练器",
    en: "SHUA Incline Seated Row (SH-G7804)",
    gif: "media/IGjKj1v.gif",
    target: ["上背部", "肱二头肌", "前臂"],
    steps: [
      "调整座椅高度与踏板位置，双脚踩实踏板上。",
      "双手窄握把手（掌心相对），背部挺直微前倾。",
      "呼气，将把手拉向腹部，同时挤压肩胛骨。",
      "顶端略停，吸气缓慢还原，重复至计划次数。"
    ],
    tips: [
      "窄握距强化上背中部厚度。",
      "用背部发力带动，不要靠手臂硬拉。",
      "还原时充分伸展肩胛。"
    ],
    mistakes: [
      "弓背或借腰摆动。",
      "耸肩或手臂代偿过多。"
    ]
  },
  {
    id: "lat-pulldown",
    cat: "back",
    equipment: "固定器械",
    level: "初级",
    name: "高位下拉",
    en: "Lat Pulldown",
    gif: "media/LEprlgG.gif",
    target: ["背阔肌", "肱二头肌", "菱形肌"],
    steps: [
      "坐在高位下拉机上，大腿固定在护垫下方。",
      "正手握杆，握距略宽于肩，挺胸、微微后倾。",
      "将横杆下拉至锁骨上方，同时收紧肩胛骨。",
      "底部停顿片刻，再缓慢放回，感受背阔肌拉伸。",
      "重复至计划次数，全程用背部发力。"
    ],
    tips: [
      "挺胸沉肩，想象“把杆拉向胸口”而不是用手臂下压。",
      "下拉至锁骨位置即可，不要拉到颈后，避免伤肩。",
      "还原阶段放慢速度，背阔肌受力时间更长。"
    ],
    mistakes: [
      "身体大幅后仰、借助惯性下拉。",
      "下拉到颈后，肩关节压力过大。"
    ]
  },
  {
    id: "seated-row",
    cat: "back",
    equipment: "固定器械",
    level: "初级",
    name: "坐姿划船",
    en: "Seated Cable Row",
    gif: "media/fUBheHs.gif",
    target: ["上背部", "肱二头肌", "前臂"],
    steps: [
      "坐在划船机上，双脚踩稳踏板，膝盖微屈。",
      "正手握柄，背部挺直，肩膀放松下沉。",
      "先将肩胛骨向后夹紧，再把手柄拉向腹部。",
      "顶峰停顿片刻，缓慢还原至手臂伸直。",
      "重复至计划次数，躯干保持稳定。"
    ],
    tips: [
      "先夹肩胛骨再拉动手柄，感受背部收缩。",
      "躯干保持中立，不要前后大幅晃动。",
      "还原时充分伸展，但不要含胸弓背。"
    ],
    mistakes: [
      "用腰部前后摆动借力。",
      "耸肩缩脖，斜方肌过度代偿。"
    ]
  },
  {
    id: "reverse-lat-pulldown",
    cat: "back",
    equipment: "固定器械",
    level: "中级",
    name: "反握高位下拉",
    en: "Reverse-Grip Lat Pulldown",
    gif: "media/ecpY0rH.gif",
    target: ["背阔肌", "肱二头肌"],
    steps: [
      "坐在高位下拉机上，大腿固定，反手握杆（掌心朝自己），握距略窄于肩。",
      "挺胸沉肩，将横杆拉向下胸部。",
      "收紧肩胛骨，顶峰停顿片刻。",
      "缓慢还原，感受背阔肌拉伸。",
      "重复至计划次数。"
    ],
    tips: [
      "反握对下背阔肌和二头刺激更多。",
      "同样避免拉到颈后，保护肩关节。",
      "还原阶段放慢速度。"
    ],
    mistakes: [
      "身体大幅后仰借力。",
      "耸肩，肱二头肌代偿过多。"
    ]
  },
  {
    id: "machine-seated-row",
    cat: "back",
    equipment: "固定器械",
    level: "初级",
    name: "器械坐姿划船",
    en: "Machine Seated Row",
    gif: "media/7I6LNUG.gif",
    target: ["上背部", "背阔肌", "肱二头肌"],
    steps: [
      "调整座椅，胸部贴紧垫子，双脚踩稳踏板。",
      "正手握柄，与肩同宽，背部挺直。",
      "将手柄拉向腹部，挤压肩胛骨。",
      "顶峰停顿片刻，缓慢还原。",
      "重复至计划次数。"
    ],
    tips: [
      "胸部垫子帮助固定躯干，专注背部发力。",
      "拉向腹部时手肘贴紧身体两侧。",
      "还原时不要含胸弓背。"
    ],
    mistakes: [
      "耸肩、缩脖子。",
      "还原过快，背部失去张力。"
    ]
  },
  {
    id: "lever-assisted-chin-up",
    cat: "back",
    equipment: "固定器械",
    level: "初级",
    name: "辅助引体向上机",
    en: "Lever Assisted Chin-Up",
    gif: "media/MaMuGH6.gif",
    target: ["背阔肌", "肱二头肌"],
    steps: [
      "根据自身体重选择配重（配重越大越省力）。",
      "膝盖跪在助力垫上（或脚踩踏板），双手握住上方横杆。",
      "呼气，背部发力将身体拉起，直至下巴过杠。",
      "吸气缓慢下放至手臂接近伸直，重复。"
    ],
    tips: [
      "先选较大配重熟悉轨迹，再逐步减重。",
      "拉起时想象胸口去贴近横杆，避免耸肩。",
      "下放要慢，全程保持背部张力。"
    ],
    mistakes: [
      "借助蹬腿惯性弹起。",
      "下放过快，肩部完全放松脱力。"
    ]
  },
  {
    id: "pull-up",
    cat: "back",
    equipment: "单双杠",
    level: "高级",
    name: "引体向上",
    en: "Pull-Up",
    gif: "media/lBDjFxJ.gif",
    target: ["背阔肌", "肱二头肌", "前臂"],
    steps: [
      "正握单杠，握距略宽于肩，手臂完全伸展悬挂。",
      "下沉肩胛骨，收紧核心。",
      "弯曲肘部，将胸口拉向单杠方向，下巴过杠。",
      "顶端停顿，缓慢下放至手臂伸直。",
      "重复至计划次数。"
    ],
    tips: [
      "先从高位下拉或弹力带辅助开始，逐步进阶。",
      "发力前先沉肩，避免耸肩代偿。",
      "下放时控制速度，背阔肌拉伸更充分。"
    ],
    mistakes: [
      "摆动身体借力，靠惯性“荡”上去。",
      "只拉至下巴高度就停止，应把胸口拉近杠。"
    ]
  },
  {
    id: "barbell-row",
    cat: "back",
    equipment: "杠铃",
    level: "中级",
    name: "杠铃划船",
    en: "Barbell Bent-Over Row",
    gif: "media/eZyBC3j.gif",
    target: ["上背部", "背阔肌", "肱二头肌"],
    steps: [
      "双脚与肩同宽站立，膝盖微屈，髋部前倾俯身，背部挺直。",
      "正手握杠，握距略宽于肩，手臂自然下垂。",
      "收紧肩胛骨，将杠铃拉向下腹部。",
      "顶峰停顿片刻，缓慢下放还原。",
      "重复至计划次数。"
    ],
    tips: [
      "躯干保持约 45° 前倾且稳定，不要随动作起伏。",
      "想象用手肘向后拉，背部发力更明显。",
      "腰背不适者可改用器械划船或减小俯身角度。"
    ],
    mistakes: [
      "弓背提拉，下背部压力大。",
      "借助惯性甩杠，用身体晃动完成动作。"
    ]
  },
  {
    id: "dumbbell-row",
    cat: "back",
    equipment: "哑铃",
    level: "初级",
    name: "哑铃俯身划船",
    en: "Dumbbell Bent-Over Row",
    gif: "media/BJ0Hz5L.gif",
    target: ["上背部", "背阔肌", "肱二头肌"],
    steps: [
      "双脚与肩同宽站立，膝盖微屈，双手持哑铃于体前。",
      "髋部铰链俯身，背部挺直，核心收紧，手臂自然下垂。",
      "呼气将哑铃拉向腹部两侧，挤压肩胛骨。",
      "顶峰停顿片刻，缓慢下放还原。",
      "重复至计划次数。"
    ],
    tips: [
      "俯身角度约 45-60°，背部保持平直。",
      "把哑铃拉向髋部方向，上背发力更明显。",
      "躯干不要左右旋转，保持稳定。"
    ],
    mistakes: [
      "背部弓起、头部下垂，腰部压力大。",
      "用惯性甩动哑铃，训练效果差。"
    ]
  },
  {
    id: "straight-arm-pulldown",
    cat: "back",
    equipment: "绳索",
    level: "中级",
    name: "直臂下拉",
    en: "Straight-Arm Pulldown",
    gif: "media/x69MAlq.gif",
    target: ["背阔肌"],
    steps: [
      "将直杆或绳索挂在高位滑轮上，面对器械站立。",
      "双手正握，手臂微屈伸直，身体微微前倾。",
      "保持手臂伸直，用背阔肌将杆向下压至大腿前侧。",
      "底部停顿片刻，缓慢还原至手臂略高于肩。",
      "重复至计划次数。"
    ],
    tips: [
      "全程手臂保持微屈，用背阔肌发力而不是三头肌。",
      "挺胸沉肩，想象“把杆压向大腿”。",
      "还原时控制速度，感受背阔肌拉伸。"
    ],
    mistakes: [
      "屈肘过度，动作变成三头下压。",
      "弓背、靠身体重量下压。"
    ]
  },
  {
    id: "lever-t-bar-row",
    cat: "back",
    equipment: "固定器械",
    level: "初级",
    name: "器械T杠划船",
    en: "Lever T Bar Row",
    gif: "media/aaXr7ld.gif",
    target: ["上背部", "肱二头肌", "前臂"],
    steps: [
      "调整座椅高度，胸口贴住靠垫，双脚踩实踏板。",
      "正握把手，握距略宽于肩，手臂自然前伸。",
      "呼气，将把手拉向躯干，同时挤压肩胛骨。",
      "顶端略停，吸气缓慢还原，重复。"
    ],
    tips: [
      "用背部发力带动，肘部贴近身体向后拉。",
      "上身贴紧靠垫，不要借腰部摆动。",
      "还原时充分伸展背部肌肉。"
    ],
    mistakes: [
      "用手臂硬拉，背部没感觉。",
      "身体离开靠垫，弓背借力。"
    ]
  },
  {
    id: "lever-high-row",
    cat: "back",
    equipment: "固定器械",
    level: "初级",
    name: "高位划船机",
    en: "Lever High Row",
    gif: "media/nZZZy9m.gif",
    target: ["上背部", "肱二头肌", "三角肌后束"],
    steps: [
      "调整座椅高度，胸口贴住靠垫坐好，双脚踩实。",
      "正握把手，握距略宽于肩，手臂前伸。",
      "呼气，将把手拉向上腹，肘部抬高，挤压肩胛。",
      "顶端略停，吸气缓慢还原，重复。"
    ],
    tips: [
      "肘部向两侧打开并抬高，强化上背与后束。",
      "全程挺胸，不要弓背。",
      "重量适中，控制节奏避免摆动。"
    ],
    mistakes: [
      "耸肩或斜方肌代偿。",
      "拉向腹部而非上腹，变成低划船。"
    ]
  },
  {
    id: "lever-pullover",
    cat: "back",
    equipment: "固定器械",
    level: "初级",
    name: "器械直臂下压",
    en: "Lever Pullover",
    gif: "media/4U7iLb5.gif",
    target: ["背阔肌", "肱三头肌", "三角肌"],
    steps: [
      "调整座椅与把手位置，背部贴紧靠垫坐好。",
      "正握把手，双臂微屈前伸。",
      "呼气，将把手拉向胸部，感受背阔肌收缩。",
      "顶端略停，吸气缓慢还原，重复。"
    ],
    tips: [
      "手臂保持微屈固定角度，用背阔肌发力。",
      "动作幅度以背阔肌收缩感为准。",
      "不要耸肩或用手臂下压。"
    ],
    mistakes: [
      "手臂弯曲过多，变成划船。",
      "耸肩或身体前倾借力。"
    ]
  },
  // ==================== 臀部训练 ====================
  {
    id: "shua-glute-bridge-bench",
    cat: "glute",
    equipment: "固定器械",
    level: "初级",
    name: "舒华提臀训练器",
    en: "SHUA Glute Trainer (SH-G7810)",
    gif: "media/qg2PGl6.gif",
    target: ["臀大肌", "腘绳肌", "核心"],
    steps: [
      "坐在器械凳边缘，上背部靠在垫上，双脚与肩同宽踩实。",
      "将杠铃杆或垫板置于髋部上方，双手扶稳。",
      "呼气，脚后跟蹬地，顶髋将臀部抬离凳面，身体呈一条直线。",
      "顶端停顿挤压臀部，吸气缓慢下放还原。",
      "重复至计划次数。"
    ],
    tips: [
      "用臀部发力顶起，不要用腰部反弓。",
      "顶端从肩到膝成一条直线。",
      "器械设计的轨道辅助保持稳定，可专注臀部收缩。"
    ],
    mistakes: [
      "腰部过度后仰代替顶髋。",
      "杠铃位置滑向腹部。"
    ]
  },
  {
    id: "lever-seated-hip-abduction",
    cat: "glute",
    equipment: "固定器械",
    level: "初级",
    name: "坐姿髋外展机",
    en: "Lever Seated Hip Abduction",
    gif: "media/CHpahtl.gif",
    target: ["臀中肌", "大腿外侧"],
    steps: [
      "坐在器械上，腰背贴紧靠垫，双腿放在膝垫内侧。",
      "调整挡板起始位置并选择合适的重量。",
      "呼气，双腿用力向外打开，感受臀部外侧发力。",
      "顶端停顿 1 秒，吸气缓慢合拢还原（配重片不碰响），重复。"
    ],
    tips: [
      "上半身贴紧靠垫，不要借腰部力量摆动。",
      "外展幅度以臀中肌有感觉为准，不必追求最大角度。",
      "还原时保持控制，配重片不要砸落出声。"
    ],
    mistakes: [
      "身体前倾或左右摇晃借力。",
      "还原过快，配重片猛烈碰撞。"
    ]
  },
  {
    id: "lever-hip-extension",
    cat: "glute",
    equipment: "固定器械",
    level: "初级",
    name: "坐姿后踢腿机",
    en: "Lever Hip Extension",
    gif: "media/OPqShYN.gif",
    target: ["臀大肌", "腘绳肌"],
    steps: [
      "调节座椅和脚踏板，背部贴紧靠背坐好，双手握紧扶手。",
      "单脚踩实脚踏板，另一侧脚踩稳地面保持稳定。",
      "臀部发力将脚踏板向后蹬出，伸展髋关节，腿向后抬起。",
      "顶端稍停感受臀部收紧，缓慢还原，单侧做完换边。"
    ],
    tips: [
      "全程背部贴紧靠背，不要弓背或塌腰。",
      "蹬出时呼气，用臀部发力而不是猛蹬。",
      "幅度以臀部完全收紧为准，不必追求抬得过高。"
    ],
    mistakes: [
      "腰部代偿、身体跟着后仰。",
      "借惯性甩腿，还原时失控。"
    ]
  },
  {
    id: "barbell-glute-bridge",
    cat: "glute",
    equipment: "杠铃",
    level: "初级",
    name: "杠铃臀桥",
    en: "Barbell Glute Bridge",
    gif: "media/qKBpF7I.gif",
    target: ["臀大肌", "腘绳肌"],
    steps: [
      "仰卧，肩胛骨靠在凳沿，双脚踩实地面，膝盖弯曲。",
      "将杠铃置于髋部，双手握杠固定（可垫毛巾护髋）。",
      "收紧核心，顶髋将臀部抬起，至膝盖-髋-肩呈一条直线。",
      "顶端用力挤压臀部 1-2 秒，缓慢下放还原。",
      "重复至计划次数。"
    ],
    tips: [
      "用臀部发力，不要靠腰部弓顶。",
      "下放时保持臀部张力，不要完全落地。",
      "感受髋部充分伸展，顶峰强烈挤压。"
    ],
    mistakes: [
      "腰部反弓代偿，臀部刺激不足。",
      "幅度太小，髋关节没有充分伸展。"
    ]
  },
  {
    id: "deadlift",
    cat: "glute",
    equipment: "杠铃",
    level: "高级",
    name: "硬拉",
    en: "Barbell Deadlift",
    gif: "media/ila4NZS.gif",
    target: ["臀大肌", "腘绳肌", "下背部"],
    steps: [
      "双脚与肩同宽站立，杠铃贴近小腿。",
      "屈髋屈膝俯身，正手握杠（握距略宽于肩）。",
      "深吸气收紧核心，保持背部平直，腿蹬地发力将杠铃拉起。",
      "站直时顶髋夹臀，不要过度后仰。",
      "屈髋屈膝控制杠铃贴腿下放回地面。"
    ],
    tips: [
      "背部始终保持中立，全程绷紧核心。",
      "想象“用脚把地面蹬开”，以腿发力为主。",
      "每次动作从地面开始，不要做半程。"
    ],
    mistakes: [
      "弓背提拉，腰椎承受巨大压力，易受伤。",
      "先用手臂拉而不是先蹬腿，动作脱节。"
    ]
  },
  {
    id: "romanian-deadlift",
    cat: "glute",
    equipment: "杠铃",
    level: "中级",
    name: "罗马尼亚硬拉",
    en: "Romanian Deadlift",
    gif: "media/wQ2c4XD.gif",
    target: ["臀大肌", "腘绳肌"],
    steps: [
      "双脚与肩同宽站立，正手握杠，握距略宽于肩。",
      "膝盖微屈，髋部向后推，将杠铃贴着腿部下放。",
      "背部保持挺直，感受腘绳肌的拉伸。",
      "下放至腘绳肌充分拉伸，顶髋站直，挤压臀部。",
      "重复至计划次数。"
    ],
    tips: [
      "杠铃全程贴近腿部，膝盖角度基本不变。",
      "重点是髋部铰链，感受腿后侧拉伸。",
      "重量适中，追求动作幅度和肌肉感受。"
    ],
    mistakes: [
      "弓背、弯腰下放，腰部压力大。",
      "膝盖弯曲过大，做成了深蹲式硬拉。"
    ]
  },
  {
    id: "kettlebell-swing",
    cat: "glute",
    equipment: "壶铃",
    level: "中级",
    name: "壶铃摆动",
    en: "Kettlebell Swing",
    gif: "media/UHJlbu3.gif",
    target: ["臀大肌", "腘绳肌", "核心"],
    steps: [
      "双脚与肩同宽站立，双手持壶铃于身体前方。",
      "微屈膝，髋部铰链将壶铃向后摆至两腿之间。",
      "利用臀部爆发力向前顶髋，将壶铃摆至胸口高度。",
      "让壶铃自然回落，重复摆动。",
      "手臂放松，全程由髋部驱动。"
    ],
    tips: [
      "手臂只是“绳子”，不要主动抬壶铃。",
      "发力来自髋部快速伸展，臀部收缩感要强。",
      "摆至胸口高度即可，不要过头顶。"
    ],
    mistakes: [
      "用肩膀和手臂把壶铃抬起来，变成前平举。",
      "下摆时弓背，腰椎弯曲。"
    ]
  },
  {
    id: "sumo-deadlift",
    cat: "glute",
    equipment: "杠铃",
    level: "中级",
    name: "相扑硬拉",
    en: "Sumo Deadlift",
    gif: "media/KgI0tqW.gif",
    target: ["臀大肌", "腘绳肌", "股四头肌"],
    steps: [
      "双脚宽站位，脚尖外展约 45°，杠铃置于脚掌中心。",
      "屈髋屈膝，挺胸直背，双手在双腿内侧握杠。",
      "腿蹬地发力，将杠铃沿小腿拉起。",
      "站直顶髋，挤压臀部，不要过度后仰。",
      "控制下放还原。"
    ],
    tips: [
      "宽站距更多刺激大腿内侧与臀部。",
      "膝盖始终与脚尖方向一致。",
      "背部全程保持中立，核心收紧。"
    ],
    mistakes: [
      "弓背提拉，腰部压力大。",
      "膝盖内扣。"
    ]
  },
  {
    id: "cable-kickback",
    cat: "glute",
    equipment: "绳索",
    level: "初级",
    name: "绳索后踢腿",
    en: "Cable Kickback",
    gif: "media/Kpajagk.gif",
    target: ["臀大肌"],
    steps: [
      "将滑轮调至低位，脚踝绑上绳索，面对器械站立。",
      "核心收紧，直背微微前倾。",
      "保持膝盖微屈，将腿向后伸直，顶峰挤压臀部。",
      "停顿片刻，缓慢还原。",
      "换腿重复。"
    ],
    tips: [
      "用臀部发力，不要用腰部。",
      "幅度不需要太大，重点是顶峰收缩。",
      "身体保持稳定，不要左右晃动。"
    ],
    mistakes: [
      "弓背、腰部代偿。",
      "靠惯性甩腿。"
    ]
  },
  {
    id: "lever-reverse-hyperextension",
    cat: "glute",
    equipment: "固定器械",
    level: "初级",
    name: "反向背伸展机",
    en: "Lever Reverse Hyperextension",
    gif: "media/Krmb3cB.gif",
    target: ["臀大肌", "腘绳肌", "下背部"],
    steps: [
      "俯卧在器械上，髋部抵住靠垫，双脚固定于踏板。",
      "双手扶稳把手，核心收紧。",
      "呼气，用臀部与腘绳发力，将双腿向后上方抬起至与地面平行。",
      "顶端停顿挤压臀部，吸气缓慢下放还原。",
      "重复至计划次数。"
    ],
    tips: [
      "用臀部发力抬腿，不要靠腰部甩动。",
      "抬腿幅度与地面平行即可，不要过度。",
      "全程动作缓慢，感受臀部收缩。"
    ],
    mistakes: [
      "腰部过度反弓代偿。",
      "抬腿过快，靠惯性完成。"
    ]
  },
  {
    id: "smith-hip-raise",
    cat: "glute",
    equipment: "史密斯机",
    level: "中级",
    name: "史密斯髋抬",
    en: "Smith Hip Raise",
    gif: "media/CqhoytW.gif",
    target: ["臀大肌", "腘绳肌", "核心"],
    steps: [
      "将史密斯杠铃调到合适高度，背部靠垫，双脚踩实与肩同宽。",
      "双手略宽于肩握杠，将杠铃置于髋部上方。",
      "呼气，脚后跟蹬地，顶髋将杠铃抬起，身体呈一条直线。",
      "顶端停顿挤压臀部，吸气缓慢下放还原。",
      "重复至计划次数。"
    ],
    tips: [
      "用臀部发力顶起，不要用腰反弓。",
      "顶端身体从肩到膝成一条直线。",
      "重量渐进增加，保持动作标准。"
    ],
    mistakes: [
      "腰部过度后仰代替顶髋。",
      "杠铃位置滑向腹部。"
    ]
  },
  // ==================== 腿部训练 ====================
  {
    id: "shua-smith-leg-press",
    cat: "leg",
    equipment: "史密斯机",
    level: "初级",
    name: "舒华坐式蹬腿训练器",
    en: "SHUA Seated Leg Press (SH-G6809)",
    gif: "media/7zdxRTl.gif",
    target: ["股四头肌", "臀大肌", "腘绳肌"],
    steps: [
      "调整座椅位置，背部贴紧靠垫，双脚与肩同宽踩实踏板。",
      "双手扶稳把手，核心收紧。",
      "呼气，将踏板蹬出至膝盖接近伸直（不锁死）。",
      "顶端略停，缓慢屈膝下放还原，重复。"
    ],
    tips: [
      "史密斯机轨道固定，轨迹更安全。",
      "膝盖始终与脚尖方向一致，不要内扣。",
      "下放至 90° 左右即可，不要过度下放。"
    ],
    mistakes: [
      "膝盖锁死或内扣。",
      "背部离开靠垫，用腰部顶。"
    ]
  },
  {
    id: "leg-press",
    cat: "leg",
    equipment: "固定器械",
    level: "初级",
    name: "腿举机",
    en: "Leg Press",
    gif: "media/10Z2DXU.gif",
    target: ["股四头肌", "臀大肌", "腘绳肌"],
    steps: [
      "调整座椅位置，背部与臀部贴紧靠垫。",
      "双脚与肩同宽踩在踏板中部，握住两侧把手。",
      "伸展双腿将踏板推离，膝盖保持微屈不锁死。",
      "缓慢屈膝下放至约 90°，再用力蹬起。",
      "重复至计划次数。"
    ],
    tips: [
      "下放时保持腰部贴紧靠垫，不要弓腰离垫。",
      "膝盖始终与脚尖方向一致，不要内扣。",
      "重量循序渐进，膝盖有不适立即停止。"
    ],
    mistakes: [
      "膝盖内扣或过度外展，关节受力不均。",
      "下放过深导致骨盆卷起、腰部悬空。"
    ]
  },
  {
    id: "leg-curl",
    cat: "leg",
    equipment: "固定器械",
    level: "初级",
    name: "腿弯举机",
    en: "Lying Leg Curl",
    gif: "media/17lJ1kr.gif",
    target: ["腘绳肌", "小腿"],
    steps: [
      "调整器械，俯卧在垫面上，膝盖对准转轴。",
      "脚后跟勾住滚垫，双手握住把手稳定身体。",
      "呼气勾起重量，脚跟向臀部方向弯曲。",
      "顶峰停顿挤压腘绳肌，吸气缓慢还原。",
      "重复至计划次数。"
    ],
    tips: [
      "髋部始终贴紧垫面，不要抬起借力。",
      "勾起时呼气，下放要缓慢控制。",
      "还原时保持腘绳肌张力，不要完全放松。"
    ],
    mistakes: [
      "靠惯性甩动重量，腘绳肌刺激不足。",
      "腰部反弓代偿，下背部压力大。"
    ]
  },
  {
    id: "leg-extension",
    cat: "leg",
    equipment: "固定器械",
    level: "初级",
    name: "腿屈伸机",
    en: "Leg Extension",
    gif: "media/my33uHU.gif",
    target: ["股四头肌"],
    steps: [
      "调整座椅与靠背，使膝关节对准器械转轴。",
      "脚背勾住滚垫，抓住两侧手柄保持稳定。",
      "伸直膝盖抬起重量，大腿前侧收缩发力。",
      "顶端停顿片刻，缓慢下放还原。",
      "重复至计划次数。"
    ],
    tips: [
      "抬腿时呼气，顶峰充分收缩股四头肌。",
      "下放速度放慢，全程保持肌肉张力。",
      "膝盖有旧伤者慎用大重量。"
    ],
    mistakes: [
      "重量过大，猛踢猛放损伤膝关节。",
      "抬臀离座借力，动作变形。"
    ]
  },
  {
    id: "hack-squat",
    cat: "leg",
    equipment: "固定器械",
    level: "中级",
    name: "哈克深蹲",
    en: "Hack Squat",
    gif: "media/Qa55kX1.gif",
    target: ["股四头肌", "臀大肌"],
    steps: [
      "调整靠背，背贴紧垫子，双脚与肩同宽踩在踏板上。",
      "解开安全锁，屈膝缓慢下放身体。",
      "下放至大腿平行地面或稍低。",
      "脚跟发力蹬起还原。",
      "重复至计划次数。"
    ],
    tips: [
      "背部全程贴紧靠垫。",
      "脚位偏高偏窄更练股四头肌，偏低偏宽更练臀。",
      "膝盖始终与脚尖方向一致。"
    ],
    mistakes: [
      "膝盖内扣。",
      "下放过深导致臀部离垫。"
    ]
  },
  {
    id: "smith-machine-squat",
    cat: "leg",
    equipment: "史密斯机",
    level: "初级",
    name: "史密斯深蹲",
    en: "Smith Machine Squat",
    gif: "media/jFtipLl.gif",
    target: ["股四头肌", "臀大肌"],
    steps: [
      "将史密斯杠铃调至肩部高度，杠置于上背斜方肌处。",
      "双脚略前于杠、与肩同宽或略宽站立，脚尖微外展。",
      "解锁杠铃，屈髋屈膝下蹲至大腿接近水平。",
      "脚跟发力站起，回到起始位后重复。"
    ],
    tips: [
      "全程腰背挺直，膝盖与脚尖方向一致。",
      "史密斯轨迹固定，可放心使用安全挡杆。",
      "下蹲吸气、起身呼气。"
    ],
    mistakes: [
      "下蹲时膝盖内扣。",
      "起身时膝盖锁死或重心压在脚尖。"
    ]
  },
  {
    id: "lever-seated-hip-adduction",
    cat: "leg",
    equipment: "固定器械",
    level: "初级",
    name: "坐姿髋内收机",
    en: "Lever Seated Hip Adduction",
    gif: "media/oHsrypV.gif",
    target: ["大腿内收肌"],
    steps: [
      "坐稳，腰背贴垫，双腿放在膝垫外侧。",
      "调整起始宽度与合适的重量。",
      "呼气，双腿向内夹拢，停顿 1 秒感受大腿内侧收缩。",
      "吸气缓慢打开还原，重复。"
    ],
    tips: [
      "膝垫抵在大腿内侧偏下位置，发力更集中。",
      "全程躯干稳定，不要弓腰。",
      "控制速度，避免猛夹猛放。"
    ],
    mistakes: [
      "用爆发力猛夹配重。",
      "还原幅度过大，造成内收肌过度拉扯。"
    ]
  },
  {
    id: "barbell-squat",
    cat: "leg",
    equipment: "杠铃",
    level: "中级",
    name: "杠铃深蹲",
    en: "Barbell Back Squat",
    gif: "media/qXTaZnJ.gif",
    target: ["股四头肌", "臀大肌", "腘绳肌"],
    steps: [
      "站立于杠下，将杠铃置于斜方肌上，双脚与肩同宽。",
      "深吸气收紧核心，出架后退两步站稳。",
      "屈髋屈膝下蹲，臀部向后向下，保持背部挺直。",
      "蹲至大腿与地面平行或稍低，膝盖与脚尖方向一致。",
      "脚后跟发力蹬地站起，呼气还原。"
    ],
    tips: [
      "全程核心收紧，躯干保持稳定不塌腰。",
      "膝盖不要内扣，重量压在脚掌中部。",
      "初学者可先用空杆练习动作模式。"
    ],
    mistakes: [
      "膝盖内扣，增加膝关节剪切力。",
      "弯腰弓背“早安式”起身，下背部风险高。"
    ]
  },
  {
    id: "dumbbell-lunge",
    cat: "leg",
    equipment: "哑铃",
    level: "初级",
    name: "哑铃箭步蹲",
    en: "Dumbbell Lunge",
    gif: "media/ecl28tP.gif",
    target: ["股四头肌", "臀大肌", "腘绳肌"],
    steps: [
      "双脚与肩同宽站立，双手持哑铃于体侧。",
      "向前迈出一步，下蹲至前腿大腿与地面平行，后膝接近地面。",
      "保持躯干直立，前脚发力蹬地回到起始位置。",
      "换腿交替进行。",
      "重复至计划次数。"
    ],
    tips: [
      "前腿膝盖与脚尖方向一致，不要内扣。",
      "躯干保持直立，不要过度前倾。",
      "动作不稳可先徒手练习，再持哑铃。"
    ],
    mistakes: [
      "前膝过度前冲，超过脚尖过多。",
      "后膝直接砸地，缺乏控制。"
    ]
  },
  {
    id: "standing-calf-raise",
    cat: "leg",
    equipment: "杠铃",
    level: "初级",
    name: "站姿提踵",
    en: "Standing Calf Raise",
    gif: "media/8ozhUIZ.gif",
    target: ["小腿"],
    steps: [
      "双脚与肩同宽站立，杠铃置于斜方肌上（或使用提踵机）。",
      "前脚掌踩实，脚后跟尽量抬高，踮起脚尖。",
      "顶端停顿挤压小腿，缓慢下放至脚后跟低于踏板。",
      "重复至计划次数。"
    ],
    tips: [
      "全程控制速度，下放时感受小腿拉伸。",
      "脚尖微内旋或外展可刺激小腿不同部位。",
      "建议 12-15 次以上，追求泵感。"
    ],
    mistakes: [
      "借助弹跳惯性完成动作。",
      "幅度太小，只做半程。"
    ]
  },
  {
    id: "split-squat",
    cat: "leg",
    equipment: "哑铃",
    level: "中级",
    name: "哑铃分腿蹲",
    en: "Dumbbell Split Squat",
    gif: "media/qx4fgX7.gif",
    target: ["股四头肌", "臀大肌"],
    steps: [
      "双手持哑铃，前脚踩地，后脚背搭在凳上。",
      "核心收紧，躯干直立，下蹲至前腿大腿平行地面。",
      "前脚发力蹬起还原。",
      "换腿重复。"
    ],
    tips: [
      "主要用前腿发力，后腿只作支撑。",
      "躯干保持直立，不要过度前倾。",
      "重心放在前脚掌中部。"
    ],
    mistakes: [
      "前膝内扣。",
      "后腿参与过多，动作变成弓步。"
    ]
  },
  {
    id: "lever-alternate-leg-press",
    cat: "leg",
    equipment: "固定器械",
    level: "初级",
    name: "交替腿举机",
    en: "Lever Alternate Leg Press",
    gif: "media/V07qpXy.gif",
    target: ["股四头肌", "腘绳肌", "臀大肌"],
    steps: [
      "调整座椅与踏板位置，背部贴紧靠垫，双脚踩实踏板。",
      "双手扶稳把手，核心收紧。",
      "单腿发力蹬出踏板至膝盖接近伸直，另一腿保持屈膝。",
      "顶端略停，缓慢还原，换另一腿交替。",
      "重复至计划次数。"
    ],
    tips: [
      "单腿交替可改善左右腿力量不平衡。",
      "膝盖始终与脚尖方向一致，不要内扣。",
      "下放时保持控制，不要弹起。"
    ],
    mistakes: [
      "膝盖锁死或内扣。",
      "靠惯性快速蹬出，顶部无停顿。"
    ]
  },
  {
    id: "lever-seated-calf-raise",
    cat: "leg",
    equipment: "固定器械",
    level: "初级",
    name: "坐姿提踵机",
    en: "Lever Seated Calf Raise",
    gif: "media/bOOdeyc.gif",
    target: ["比目鱼肌", "腓肠肌"],
    steps: [
      "调整座椅高度，前脚掌踩在踏板上，脚后跟悬空。",
      "双膝约 90°，将垫板压在膝盖上方，双手扶稳。",
      "脚后跟尽量向下放低，感受小腿拉伸。",
      "用力踮起脚尖，将脚跟抬至最高点，顶端停顿。",
      "缓慢下放还原，重复。"
    ],
    tips: [
      "坐姿屈膝主要刺激比目鱼肌（小腿深层）。",
      "全程动作缓慢，顶峰充分收缩。",
      "重量渐进，保持踝关节稳定。"
    ],
    mistakes: [
      "脚跟下放过快，借助弹力。",
      "幅度太小，没有充分拉伸与收缩。"
    ]
  },
  {
    id: "lever-standing-calf-raise",
    cat: "leg",
    equipment: "固定器械",
    level: "初级",
    name: "站姿提踵机",
    en: "Lever Standing Calf Raise",
    gif: "media/ykUOVze.gif",
    target: ["腓肠肌", "比目鱼肌"],
    steps: [
      "调整器械高度，肩部抵住垫板，双脚与肩同宽踩实。",
      "双手扶稳把手，身体保持直立。",
      "脚跟尽量向下放低，感受小腿拉伸。",
      "用力踮起脚尖，将脚跟抬至最高点，顶端停顿。",
      "缓慢下放还原，重复。"
    ],
    tips: [
      "站姿直膝主要刺激腓肠肌（小腿表层）。",
      "全程身体不要前后晃动。",
      "顶峰停顿 1-2 秒，强化收缩。"
    ],
    mistakes: [
      "屈膝借力，变成深蹲提踵。",
      "下放过快，靠弹力完成。"
    ]
  },
  // ==================== 手臂训练 ====================
  {
    id: "shua-tricep-extension",
    cat: "arm",
    equipment: "史密斯机",
    level: "中级",
    name: "舒华三头肌训练器",
    en: "SHUA Tricep Extension (SH-G6808)",
    gif: "media/o8aOcrz.gif",
    target: ["肱三头肌", "三角肌"],
    steps: [
      "调整座椅高度使杠铃杆与肩齐平，背部贴紧靠垫。",
      "正握略宽于肩握杠，推起至头顶，手臂伸直。",
      "缓慢下放杠铃至前额上方，肘部微向外，上臂保持固定。",
      "停顿片刻，呼气伸肘还原，重复至计划次数。"
    ],
    tips: [
      "上臂保持固定，只有前臂活动。",
      "史密斯机轨迹固定，专注三头收缩。",
      "下放至前额高度即可，不要过低。"
    ],
    mistakes: [
      "上臂晃动或肘部过度外展。",
      "下放过快失去控制。"
    ]
  },
  {
    id: "lever-bicep-curl",
    cat: "arm",
    equipment: "固定器械",
    level: "初级",
    name: "坐姿弯举机",
    en: "Lever Bicep Curl",
    gif: "media/q6y3OhV.gif",
    target: ["肱二头肌"],
    steps: [
      "坐在弯举机上，上臂贴住前垫，肘部对准器械转轴。",
      "双手反握把手，呼气向上弯举至肱二头肌完全收缩。",
      "顶端略停，吸气缓慢下放还原，重复。"
    ],
    tips: [
      "上臂固定不动，避免耸肩或身体后仰借力。",
      "下放至手臂接近伸直即可，保持肌肉张力。",
      "顶峰收缩 1 秒效果更好。"
    ],
    mistakes: [
      "借助惯性甩起重量。",
      "下放过快、肘部离开垫板。"
    ]
  },
  {
    id: "lever-triceps-extension",
    cat: "arm",
    equipment: "固定器械",
    level: "初级",
    name: "坐姿三头屈伸机",
    en: "Lever Triceps Extension",
    gif: "media/Ser9eQp.gif",
    target: ["肱三头肌"],
    steps: [
      "调节座椅高度，背部靠紧靠垫坐好。",
      "正手握住把手，双臂在体前伸直，肘部与肩同高。",
      "大臂保持固定，弯曲手肘将把手缓慢放向额头方向。",
      "底部稍停，肱三头肌发力将把手推回起始位置，重复。"
    ],
    tips: [
      "大臂全程夹紧固定，只动肘关节。",
      "下放缓慢感受三头肌拉伸，推起时呼气。",
      "座椅高度以动作全程不耸肩为准。"
    ],
    mistakes: [
      "耸肩或大臂晃动借力。",
      "推起时甩臂、速度过快。"
    ]
  },
  {
    id: "barbell-curl",
    cat: "arm",
    equipment: "杠铃",
    level: "初级",
    name: "杠铃弯举",
    en: "Barbell Curl",
    gif: "media/25GPyDY.gif",
    target: ["肱二头肌", "前臂"],
    steps: [
      "站直，双脚与肩同宽，反手握住杠铃，手掌朝前。",
      "上臂贴紧身体两侧，肘部位置固定。",
      "呼气弯举杠铃至肩部高度，收缩肱二头肌。",
      "顶峰停顿 1 秒，吸气缓慢下放还原。",
      "重复至计划次数。"
    ],
    tips: [
      "身体不要前后摇摆，拒绝借力。",
      "下放至手臂接近伸直，保持肌肉张力。",
      "手腕保持中立，不要弯腕代偿。"
    ],
    mistakes: [
      "甩杠铃借力，二头肌刺激大打折扣。",
      "肘部前移，把弯举做成前平举。"
    ]
  },
  {
    id: "dumbbell-curl",
    cat: "arm",
    equipment: "哑铃",
    level: "初级",
    name: "哑铃弯举",
    en: "Dumbbell Curl",
    gif: "media/NbVPDMW.gif",
    target: ["肱二头肌", "前臂"],
    steps: [
      "站直，双手持哑铃于体侧，手掌朝前，手臂自然下垂。",
      "上臂保持固定，呼气弯举哑铃至肩部高度。",
      "顶峰停顿挤压二头肌，吸气缓慢下放。",
      "可以左右交替进行，保持稳定节奏。",
      "重复至计划次数。"
    ],
    tips: [
      "弯举至顶峰时可轻微外旋手腕（如使用旋转弯举），刺激更强。",
      "还原到手臂完全伸展，保持张力。",
      "选择能标准完成 8-12 次的重量。"
    ],
    mistakes: [
      "肘部前移借力。",
      "身体前后摆动，甩动哑铃完成动作。"
    ]
  },
  {
    id: "pushdown",
    cat: "arm",
    equipment: "绳索",
    level: "初级",
    name: "绳索下压",
    en: "Cable Triceps Pushdown",
    gif: "media/dU605di.gif",
    target: ["肱三头肌", "前臂"],
    steps: [
      "将绳索附件挂在高位滑轮上。",
      "面对器械站立，双脚与肩同宽，膝盖微屈。",
      "双手对握绳索，上臂贴紧身体两侧并保持固定。",
      "呼气伸直肘关节将绳索下压至手臂完全伸直。",
      "停顿片刻，吸气缓慢还原。"
    ],
    tips: [
      "上臂全程固定，只有前臂在动。",
      "下压到底时可将绳索两端向两侧分开，加深刺激。",
      "身体微微前倾、核心收紧，防止借力。"
    ],
    mistakes: [
      "肘部前后移动，用身体重量下压。",
      "耸肩、背部代偿，三头肌受力变小。"
    ]
  },
  {
    id: "hammer-curl",
    cat: "arm",
    equipment: "哑铃",
    level: "初级",
    name: "哑铃锤式弯举",
    en: "Hammer Curl",
    gif: "media/slDvUAU.gif",
    target: ["肱二头肌", "肱肌", "前臂"],
    steps: [
      "站直，双手持哑铃，掌心相对（锤式握法）。",
      "上臂固定，将哑铃弯举至肩部高度。",
      "顶峰停顿挤压，缓慢下放还原。",
      "重复至计划次数。"
    ],
    tips: [
      "锤式握法对前臂和肱肌刺激更多，手腕更舒适。",
      "全程肘部位置固定，身体不摆动。",
      "可双臂同时或交替进行。"
    ],
    mistakes: [
      "身体后仰借力。",
      "手腕弯曲代偿。"
    ]
  },
  {
    id: "ez-curl",
    cat: "arm",
    equipment: "杠铃",
    level: "初级",
    name: "EZ杠弯举",
    en: "EZ-Bar Curl",
    gif: "media/6TG6x2w.gif",
    target: ["肱二头肌", "前臂"],
    steps: [
      "站直，反握 EZ 杠，握距与肩同宽。",
      "上臂贴紧身体，将杠弯举至肩部高度。",
      "顶峰挤压二头肌，缓慢下放还原。",
      "重复至计划次数。"
    ],
    tips: [
      "EZ 杠的弯折角度对手腕更友好。",
      "肘部位置固定，身体不要晃动。",
      "下放至手臂接近伸直，保持张力。"
    ],
    mistakes: [
      "甩杠借力。",
      "肘部前移。"
    ]
  },
  {
    id: "skull-crusher",
    cat: "arm",
    equipment: "杠铃",
    level: "中级",
    name: "仰卧臂屈伸",
    en: "Skull Crusher",
    gif: "media/h8LFzo9.gif",
    target: ["肱三头肌"],
    steps: [
      "仰卧于凳，双手与肩同宽正握杠铃，手臂伸直至胸部上方。",
      "上臂保持固定，屈肘将杠铃下放至前额上方。",
      "停顿片刻，伸肘还原。",
      "重复至计划次数。"
    ],
    tips: [
      "上臂保持垂直固定，只有前臂在活动。",
      "肘部朝前或微内收，避免外展。",
      "先用空杆熟悉轨迹，注意安全。"
    ],
    mistakes: [
      "肘部外展、上臂晃动。",
      "下放过快失去控制。"
    ]
  },
  {
    id: "lever-overhand-triceps-dip",
    cat: "arm",
    equipment: "固定器械",
    level: "初级",
    name: "器械正握臂屈伸",
    en: "Lever Overhand Triceps Dip",
    gif: "media/D5yqP2p.gif",
    target: ["肱三头肌", "胸大肌", "三角肌"],
    steps: [
      "调整器械高度，双膝跪在垫板上，双手正握把手。",
      "手臂伸直支撑，肘部贴近身体。",
      "缓慢屈肘下降，上臂贴紧体侧，至前臂与地面平行。",
      "呼气伸肘推起还原，重复至计划次数。"
    ],
    tips: [
      "肘部贴紧身体，重点刺激三头。",
      "下降速度放慢，控制动作节奏。",
      "辅助重量从大到小渐进。"
    ],
    mistakes: [
      "肘部过度外展，肩部压力大。",
      "下降过快或耸肩。"
    ]
  },
  {
    id: "lever-preacher-curl",
    cat: "arm",
    equipment: "固定器械",
    level: "初级",
    name: "牧师弯举机",
    en: "Lever Preacher Curl",
    gif: "media/b6hQYMb.gif",
    target: ["肱二头肌", "前臂"],
    steps: [
      "调整座椅高度，上臂完全贴住斜垫，反手握把手。",
      "背部挺直，肘部固定于垫面。",
      "呼气，前臂向上弯举，收缩肱二头肌。",
      "顶端停顿挤压，吸气缓慢下放还原，重复。"
    ],
    tips: [
      "上臂全程贴紧垫面，不要离开。",
      "下放时充分伸展二头，底部不要完全松劲。",
      "重量宜轻，避免手腕过度弯折。"
    ],
    mistakes: [
      "上臂离开垫面，借身体晃动弯举。",
      "下放过快，二头失去张力。"
    ]
  },
  // ==================== 核心训练 ====================
  {
    id: "lever-back-extension",
    cat: "core",
    equipment: "固定器械",
    level: "初级",
    name: "罗马椅背屈伸",
    en: "Back Extension",
    gif: "media/zhMwOwE.gif",
    target: ["竖脊肌", "臀大肌"],
    steps: [
      "俯卧在罗马椅上，髋部抵住前端软垫，脚踝固定在后方挡板下。",
      "双手交叉抱胸或轻放于耳侧。",
      "以髋为轴，躯干缓慢前屈下放，保持背部平直。",
      "臀部与下背发力抬起躯干至与腿呈一条直线。",
      "顶端略停，缓慢还原后重复。"
    ],
    tips: [
      "顶端不要过度后仰超伸腰椎。",
      "全程收紧臀部与核心。",
      "新手自重即可，进阶可胸前抱杠铃片。"
    ],
    mistakes: [
      "用爆发力甩起身体。",
      "下放弓背、起身时腰部反弓过度。"
    ]
  },
  {
    id: "lever-seated-crunch",
    cat: "core",
    equipment: "固定器械",
    level: "初级",
    name: "坐姿卷腹机",
    en: "Lever Seated Crunch",
    gif: "media/ZnJHhMk.gif",
    target: ["腹直肌"],
    steps: [
      "坐在卷腹机上，双脚踩稳，双手握把置于胸前。",
      "呼气，腹部发力使躯干向前卷曲，将胸垫压向大腿方向。",
      "顶端收紧腹肌 1 秒，吸气缓慢还原，重复。"
    ],
    tips: [
      "用腹肌卷曲发力，而不是靠手臂下压。",
      "还原时不要让配重片完全落回，保持腹肌张力。",
      "调好胸垫高度，让发力集中在腹肌上段。"
    ],
    mistakes: [
      "用手臂猛拉或身体后仰借力。",
      "幅度过大，腰部代偿。"
    ]
  },
  {
    id: "crunch",
    cat: "core",
    equipment: "徒手",
    level: "初级",
    name: "卷腹",
    en: "Crunch",
    gif: "media/TFqbd8t.gif",
    target: ["腹直肌"],
    steps: [
      "仰卧，膝盖弯曲，双脚踩实地面。",
      "双手轻放于耳侧或胸前，肘部朝外。",
      "收紧腹肌，将肩胛骨抬离地面，向膝盖方向卷起。",
      "顶部停顿片刻，缓慢下放还原。",
      "重复至计划次数。"
    ],
    tips: [
      "用腹肌“卷”起上半身，而不是靠颈部发力。",
      "下背部始终贴地，起身幅度不必过大。",
      "起落都要缓慢控制，避免惯性。"
    ],
    mistakes: [
      "双手抱头用力拉拽颈部，颈椎压力大。",
      "起身过高变成仰卧起坐，腰部离地代偿。"
    ]
  },
  {
    id: "plank",
    cat: "core",
    equipment: "徒手",
    level: "初级",
    name: "平板支撑",
    en: "Forearm Plank",
    gif: "media/VBAWRPG.gif",
    target: ["腹肌", "核心"],
    steps: [
      "俯卧，前臂撑地，肘部位于肩部正下方。",
      "双腿伸直，脚趾撑地。",
      "收紧核心与臀部，将身体抬离地面，从头到脚呈一条直线。",
      "保持均匀呼吸，维持该姿势至规定时间。",
      "缓慢放下身体还原。"
    ],
    tips: [
      "收紧腹部和臀部，想象“把肚脐收向脊柱”。",
      "质量比时长更重要，动作变形就停下休息。",
      "可从 20-30 秒开始，逐步延长。"
    ],
    mistakes: [
      "塌腰，腰椎过度受压。",
      "撅臀过高，核心失去张力。"
    ]
  },
  {
    id: "russian-twist",
    cat: "core",
    equipment: "徒手",
    level: "初级",
    name: "俄罗斯转体",
    en: "Russian Twist",
    gif: "media/XVDdcoj.gif",
    target: ["腹斜肌", "腹肌"],
    steps: [
      "坐姿，屈膝，身体微微后仰，收紧核心。",
      "双脚离地（初学者可踩地），双手合拢于胸前。",
      "左右交替扭转躯干，感受侧腹收缩。",
      "重复至计划次数。"
    ],
    tips: [
      "用核心扭转躯干，而不是靠手臂摆动。",
      "可手持哑铃片或药球增加负荷。",
      "保持背部挺直，不要弓背。"
    ],
    mistakes: [
      "只转手臂不转躯干。",
      "坐姿过低、弓背含胸。"
    ]
  },
  {
    id: "hanging-leg-raise",
    cat: "core",
    equipment: "单双杠",
    level: "高级",
    name: "悬垂举腿",
    en: "Hanging Leg Raise",
    gif: "media/I3tsCnC.gif",
    target: ["腹直肌", "髋屈肌"],
    steps: [
      "悬挂于单杠，手臂伸直，收紧核心。",
      "保持双腿伸直（或微屈），抬至与地面平行或更高。",
      "顶峰停顿片刻，缓慢下放还原。",
      "重复至计划次数。"
    ],
    tips: [
      "用腹肌卷起骨盆发力，而不是甩腿。",
      "初学者可先做屈膝举腿。",
      "下放控制速度，避免身体摆动。"
    ],
    mistakes: [
      "摆动身体借力。",
      "弓背，只靠髋屈肌发力。"
    ]
  },
  {
    id: "side-plank",
    cat: "core",
    equipment: "徒手",
    level: "初级",
    name: "侧平板支撑",
    en: "Side Plank",
    gif: "media/VO2qeJg.gif",
    target: ["腹斜肌", "核心"],
    steps: [
      "侧卧，前臂撑地，肘部位于肩部正下方。",
      "收紧核心，将髋部抬离地面，从头到脚呈一条直线。",
      "保持姿势至规定时间，缓慢放下。",
      "换边重复。"
    ],
    tips: [
      "髋部不要下沉或撅起，身体保持一条直线。",
      "保持均匀呼吸，不要憋气。",
      "可从 20-30 秒开始，逐步延长。"
    ],
    mistakes: [
      "塌腰或髋部下坠。",
      "头部过度前伸。"
    ]
  },
  {
    id: "lever-seated-leg-raise-crunch",
    cat: "core",
    equipment: "固定器械",
    level: "初级",
    name: "坐姿抬腿卷腹机",
    en: "Lever Seated Leg Raise Crunch",
    gif: "media/PQ2AtC3.gif",
    target: ["腹直肌", "髋屈肌"],
    steps: [
      "坐于器械上，背部贴紧靠垫，双脚踩在踏板上。",
      "双手扶稳把手或侧杆，核心收紧。",
      "呼气，屈膝抬腿向胸部靠拢，同时躯干前卷。",
      "顶端停顿挤压腹部，吸气缓慢还原，重复。"
    ],
    tips: [
      "用腹部收缩带动卷起，不要只抬腿。",
      "动作缓慢控制，避免用惯性。",
      "顶端充分挤压腹肌。"
    ],
    mistakes: [
      "只抬腿不卷腹，髋屈肌代偿。",
      "还原过快，腹部无持续张力。"
    ]
  },
  {
    id: "lever-kneeling-twist",
    cat: "core",
    equipment: "固定器械",
    level: "初级",
    name: "器械跪姿扭转",
    en: "Lever Kneeling Twist",
    gif: "media/sZOR9EV.gif",
    target: ["腹直肌", "腹斜肌", "下背部"],
    steps: [
      "调整座椅高度，双膝跪在垫上，上身前倾面对器械。",
      "双手扶稳把手，核心收紧。",
      "呼气，躯干向一侧旋转至舒适幅度。",
      "停顿片刻，缓慢回正，换另一侧交替。",
      "重复至计划次数。"
    ],
    tips: [
      "以腹斜肌发力带动旋转，不要用腰部甩。",
      "旋转幅度以舒适为准，不要过度。",
      "骨盆保持稳定，不要跟着转动。"
    ],
    mistakes: [
      "用惯性快速甩动躯干。",
      "骨盆跟着转动，动作变形。"
    ]
  },
  // ==================== 有氧训练 ====================
  {
    id: "treadmill",
    cat: "cardio",
    equipment: "有氧器械",
    level: "初级",
    name: "跑步机",
    en: "Treadmill",
    gif: "media/rjiM4L3.gif",
    target: ["心肺功能", "股四头肌", "小腿"],
    steps: [
      "将跑步机的坡度调到适合的强度，从低速度开始。",
      "双脚分开与肩同宽站立在跑带上，先低速热身 3-5 分钟。",
      "以舒适的配速行走或慢跑，保持躯干直立、目视前方。",
      "全程收紧核心，保持背部挺直，双臂自然摆动。",
      "完成训练前逐步降低坡度和速度，待心率平复后再下机。"
    ],
    tips: [
      "上机前先做低强度热身，再逐步提速，避免突然加速。",
      "不要在跑步时紧握扶手，仅起步或疲劳时轻扶。",
      "结束时务必先减速，防止下机瞬间头晕摔倒。"
    ],
    mistakes: [
      "低头看屏幕，导致颈肩紧张、含胸驼背。",
      "速度设定过高、全程抓扶手借力，降低训练效果且易伤膝关节。"
    ]
  },
  {
    id: "elliptical",
    cat: "cardio",
    equipment: "有氧器械",
    level: "初级",
    name: "椭圆机",
    en: "Elliptical Trainer",
    gif: "media/rjtuP6X.gif",
    target: ["心肺功能", "股四头肌", "臀部"],
    steps: [
      "调整阻力与坡度至合适水平，踩上踏板并轻握手柄。",
      "脚掌向下踩踏，同时将手柄向身体方向拉动。",
      "交替推拉，模拟走路或跑步的连贯动作。",
      "保持匀速节奏，核心始终收紧，身体稳定。",
      "下机前逐渐降低强度与速度，再做放松。"
    ],
    tips: [
      "脚掌全程贴紧踏板，脚跟不要抬起。",
      "配合手臂推拉可以带动全身，训练更高效。",
      "膝盖始终与脚尖方向一致，避免内扣。"
    ],
    mistakes: [
      "身体后仰过度、骨盆前倾，腰部压力增大。",
      "双手死死抓紧把手、把体重压在把手上。"
    ]
  },
  {
    id: "stationary-bike",
    cat: "cardio",
    equipment: "有氧器械",
    level: "初级",
    name: "动感单车",
    en: "Stationary Bike",
    gif: "media/H1PESYI.gif",
    target: ["心肺功能", "股四头肌", "小腿"],
    steps: [
      "调整座椅高度：踏板踩到最低点时膝盖微屈约 15-30°。",
      "将脚固定在踏板上，脚尖朝前，以舒适节奏开始踩踏。",
      "保持稳定踏频，根据体感逐渐增加阻力。",
      "核心收紧、背部平直，不要耸肩。",
      "结束后逐步减小阻力并减速，再下车拉伸双腿。"
    ],
    tips: [
      "座椅不宜过低或过前，否则膝盖压力过大。",
      "用前脚掌发力踩踏，节奏比阻力更重要。",
      "站立骑行时重心放在踏板上，避免车身左右摇摆。"
    ],
    mistakes: [
      "座椅高度不合适导致膝盖疼痛。",
      "站姿骑行时弯腰弓背、靠手臂撑住车把。"
    ]
  },
  {
    id: "stepmill",
    cat: "cardio",
    equipment: "有氧器械",
    level: "初级",
    name: "登山机",
    en: "Stair Climber",
    gif: "media/j9Q5crt.gif",
    target: ["心肺功能", "臀大肌", "股四头肌"],
    steps: [
      "将机器速度调整到较低档位，走上台阶并扶稳扶手。",
      "双腿交替踩踏，全脚掌落在台阶上。",
      "保持躯干直立，核心收紧，不要弯腰驼背。",
      "按计划持续行走，逐步适应后增加速度。",
      "结束前降低速度，安全下机后做拉伸放松。"
    ],
    tips: [
      "扶手只用于保持平衡，不要用手臂支撑身体重量。",
      "挺胸抬头，让臀腿发力，效果更好。",
      "初练者从低速开始，每次 10-15 分钟为宜。"
    ],
    mistakes: [
      "弯腰驼背，靠扶手“爬”上去，训练效果大打折扣。",
      "追求速度一步跨两个台阶，容易失去平衡。"
    ]
  },
  {
    id: "jump-rope",
    cat: "cardio",
    equipment: "跳绳",
    level: "初级",
    name: "跳绳",
    en: "Jump Rope",
    gif: "media/e1e76I2.gif",
    target: ["心肺功能", "小腿"],
    steps: [
      "双手握绳柄，绳置于身后，双脚并拢。",
      "用手腕发力摇绳，绳过脚时轻盈跳起。",
      "前脚掌着地，保持节奏。",
      "持续跳跃至规定时间或次数。"
    ],
    tips: [
      "用手腕摇绳，而不是整个手臂。",
      "起跳高度刚好让绳通过即可，不要跳太高。",
      "膝盖微屈缓冲，减少对关节的冲击。"
    ],
    mistakes: [
      "跳得过高、落地过重。",
      "手臂大幅甩动、绳子缠绕打结。"
    ]
  },
  {
    id: "burpee",
    cat: "cardio",
    equipment: "徒手",
    level: "中级",
    name: "波比跳",
    en: "Burpee",
    gif: "media/dK9394r.gif",
    target: ["心肺功能", "全身"],
    steps: [
      "站立，下蹲，双手撑地。",
      "双脚向后跳成俯卧撑位，做一个俯卧撑。",
      "双脚跳回，起身向上跳起，双臂举过头顶。",
      "落地后立即开始下一次。",
      "重复至计划次数。"
    ],
    tips: [
      "动作连贯，保持呼吸节奏。",
      "初学者可省去俯卧撑部分降阶。",
      "落地轻缓，膝盖微屈缓冲。"
    ],
    mistakes: [
      "俯卧撑位塌腰。",
      "追求速度导致动作变形。"
    ]
  },
  // ==================== 拉伸放松 ====================
  {
    id: "standing-hamstring-stretch",
    cat: "stretch",
    equipment: "徒手",
    level: "初级",
    name: "站姿体前屈触地拉伸",
    en: "Standing Toe Touch Stretch",
    gif: "media/BbfB8Gb.gif",
    target: ["腘绳肌", "臀大肌", "下背部"],
    relax: ["leg", "glute", "back", "warmup"],
    steps: [
      "站立，双脚并拢或与肩同宽，膝盖微屈。",
      "呼气，从髋部折叠向前俯身，双手尽量向脚尖或地面延伸。",
      "放松头颈与肩部，感受大腿后侧与下背的拉伸。",
      "保持 20-30 秒，缓慢起身还原。"
    ],
    tips: [
      "全程膝盖微屈即可，不必锁死，更多靠髋部折叠。",
      "柔韧性不足时双手扶小腿或膝盖即可，不必强触地。",
      "配合呼气缓慢加深拉伸。"
    ],
    mistakes: [
      "膝盖过度锁死，靠弓背硬压。",
      "弹振式下压导致下背紧张。"
    ]
  },
  {
    id: "standing-quad-stretch",
    cat: "stretch",
    equipment: "徒手",
    level: "初级",
    name: "站姿股四头肌拉伸",
    en: "Standing Quad Stretch",
    gif: "media/tFGKm99.gif",
    target: ["股四头肌", "髋屈肌"],
    relax: ["leg", "glute", "warmup"],
    steps: [
      "站立，左手扶墙或固定物保持平衡。",
      "右腿向后屈膝，右手抓住右脚踝向臀部方向拉。",
      "双膝并拢，腹部微收，感受大腿前侧拉伸，保持 20-30 秒。",
      "缓慢还原，换另一侧重复。"
    ],
    tips: [
      "保持躯干直立，不要前倾或后仰。",
      "膝盖指向正下方，避免外撇。",
      "若是拉绳辅助，同样轻缓用力。"
    ],
    mistakes: [
      "腰部过度后弓代替腿部拉伸。",
      "脚踝靠臀部过近造成不适仍强压。"
    ]
  },
  {
    id: "standing-glute-stretch",
    cat: "stretch",
    equipment: "徒手",
    level: "初级",
    name: "站姿单脚臀腿拉伸",
    en: "Standing Hip & Glute Stretch",
    gif: "media/sU5BrfP.gif",
    target: ["臀大肌", "腘绳肌"],
    relax: ["glute", "leg"],
    steps: [
      "站立，双脚分开与肩同宽，可单手扶墙保持平衡。",
      "右腿屈膝抬起，右踝搭在左膝上方（四字交叉）。",
      "屈髋下蹲，身体略微后坐，感受右侧臀部拉伸。",
      "保持 20-30 秒，起身换另一侧。"
    ],
    tips: [
      "动作全程保持上身直立，不要弓腰驼背。",
      "下蹲幅度以臀部有明显拉伸感为准，不必蹲太低。",
      "扶墙或扶椅保持平衡，更安全更稳。"
    ],
    mistakes: [
      "弓背或身体前倾过度。",
      "膝盖向内错位或脚踝搭得过高。"
    ]
  },
  {
    id: "wall-calf-stretch",
    cat: "stretch",
    equipment: "徒手",
    level: "初级",
    name: "扶墙小腿拉伸",
    en: "Calf Stretch with Hands Against Wall",
    gif: "media/PzNxakt.gif",
    target: ["腓肠肌", "比目鱼肌"],
    relax: ["leg", "cardio", "warmup"],
    steps: [
      "面对墙壁站立，双手撑墙，与肩同高。",
      "左脚向后迈一大步，脚跟踩实地面，前脚屈膝。",
      "重心前移、双手撑墙，身体前倾，感受后腿小腿后侧拉伸。",
      "保持 20-30 秒，缓慢还原换另一侧。"
    ],
    tips: [
      "后脚跟始终踩实地面，不要踮起。",
      "身体保持正直，前腿可自然屈膝。",
      "拉伸感明显时配合呼气加深。"
    ],
    mistakes: [
      "后脚跟离地或膝盖过度弯曲。",
      "弓腰驼背导致重心不稳。"
    ]
  },
  {
    id: "standing-pec-stretch",
    cat: "stretch",
    equipment: "徒手",
    level: "初级",
    name: "站姿胸肌拉伸",
    en: "Standing Pec Stretch",
    gif: "media/chfnQnM.gif",
    target: ["胸大肌", "背阔肌"],
    relax: ["chest", "shoulder"],
    steps: [
      "站立，双脚与肩同宽。",
      "双臂伸直前平举，双手交叉、掌心翻转朝外。",
      "缓慢向上并向后抬起双臂，感受胸肌与肩背的拉伸。",
      "保持 15-30 秒，放松后换另一侧。"
    ],
    tips: [
      "动作缓慢，幅度以舒适为准。",
      "肩胛骨轻收，避免耸肩。",
      "若单侧更紧可单独交替拉伸。"
    ],
    mistakes: [
      "耸肩导致肩颈紧张。",
      "抬臂过猛引发肩部不适。"
    ]
  },
  {
    id: "rear-deltoid-stretch",
    cat: "stretch",
    equipment: "徒手",
    level: "初级",
    name: "肩后侧拉伸",
    en: "Rear Deltoid Stretch",
    gif: "media/xifhB5W.gif",
    target: ["三角肌后束", "斜方肌"],
    relax: ["shoulder", "back"],
    steps: [
      "站立，肩背放松。",
      "右臂伸直横过胸前，左手扶住右肘。",
      "左手轻拉右臂靠近左肩，感受右肩后侧拉伸。",
      "保持 15-30 秒，换另一侧。"
    ],
    tips: [
      "保持双肩水平，不要耸肩。",
      "拉伸力度温和，以舒适为准。",
      "配合呼气加深拉伸。"
    ],
    mistakes: [
      "耸肩或身体跟着转动。",
      "用力过猛牵拉肩关节。"
    ]
  },
  {
    id: "overhead-triceps-stretch",
    cat: "stretch",
    equipment: "徒手",
    level: "初级",
    name: "站姿三头肌拉伸",
    en: "Overhead Triceps Stretch",
    gif: "media/Z5YStHW.gif",
    target: ["肱三头肌"],
    relax: ["arm", "chest"],
    steps: [
      "站立，双脚与肩同宽，可单手扶墙保持平衡。",
      "右臂上举过头并屈肘，手掌靠近后背肩胛处。",
      "左手轻扶右肘，向头后轻压，感受三头肌拉伸。",
      "保持 15-30 秒，换另一侧。"
    ],
    tips: [
      "保持躯干直立稳定。",
      "手扶肘部轻压即可，不要扳压过猛。",
      "若够不到后背，可先用毛巾辅助。"
    ],
    mistakes: [
      "身体侧弯借力。",
      "压肘过猛引起肩肘不适。"
    ]
  },
  {
    id: "standing-side-stretch",
    cat: "stretch",
    equipment: "徒手",
    level: "初级",
    name: "站姿体侧拉伸",
    en: "Standing Lateral Stretch",
    gif: "media/1jXLYEw.gif",
    target: ["背阔肌", "躯干侧链", "斜方肌中下束"],
    relax: ["back", "core", "arm"],
    steps: [
      "站立，双脚与肩同宽，双臂自然下垂。",
      "右手叉腰或自然垂放，左臂伸直举过头顶。",
      "身体向右侧屈，左侧躯干有明显拉伸感。",
      "保持 15-30 秒，缓慢还原换另一侧。"
    ],
    tips: [
      "身体保持在一个平面内侧屈，不要前倾或后仰。",
      "下肢稳实不动，靠髋部上方侧屈带动。",
      "配合呼气缓慢加深侧屈幅度。"
    ],
    mistakes: [
      "骨盆跟着转动，侧屈变成弯腰。",
      "耸肩或颈部过度侧偏代偿。"
    ]
  }
];
